...
- [ ] **Photos — split by keepsake vs. log data** (owner call, 2026-07-24):
  - ~~**Progress pics → PhotoKit reference + thumbnail.**~~ **SUPERSEDED 2026-08-12** — owner-accepted amendment, built at migration 0035 (`docs/progress-photos-subapp.md` §1). The letter below stands as the *reasoning*, and its intent survives: iCloud Photos still owns the original at full quality and ARC's copy is deliberately not a keepsake. What could not survive is the mechanism. **(a)** Following a PhotoKit reference needs `expo-media-library`, which is not installed and is in no planned build — so a stored reference would be a reference nothing can follow. **(b)** The restore finding two bullets down is fatal on its own, and its proposed fix is unavailable: **no Expo module exposes `PHCloudIdentifier`** (verified 2026-08-12 — expo-media-library's API is `localIdentifier`-based throughout). So the always-stored artefact is a **working copy**: 1600px on the longest edge, JPEG q0.7, ~180–350 KB, made at pick time. ~0.5 GB/decade at three poses a week, against the ~3 GB/decade food-photo budget this same phase already accepted. `asset_id` is still stored, as dormant provenance: never read for behaviour, it powers duplicate detection and is the hook a future MediaLibrary upgrade would re-link through.
  ...
  - Storage consequence: in-app food copies ride inside ARC's encrypted backup (~3 GB/decade); referenced progress pics don't (already in iCloud Photos). This split is *why* the app's backup blob stays small. **Revised with the amendment above (owner call, 2026-08-12): the progress-photo working copies and the `is_important` full-resolution copies DO ride the snapshot** (~0.5 GB/decade plus a handful of deliberate, individually-priced originals). Excluding them re-creates the everything-dangles restore hole the audit finding names. Camera-roll originals still never ride — iCloud Photos owns those — which is the half of the split that keeps the blob small.
...
- ✅ **Photo storage:** the keepsake-vs-log split above — **amended 2026-08-12**: compress food logs (shipped, 0033), and keep a compressed **working copy** of progress pics rather than a PhotoKit reference (shipped, 0035). iCloud Photos still owns the full-quality original of a progress pic; what changed is that ARC no longer relies on being able to reach it.
