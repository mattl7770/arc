...
`higherIsBetter` is `null` on **43 of 65** — the other 22 split **17 `0`** (lower is better) and **5 `1`** (higher is better). *(Re-derived from `BIOMARKER_SEED` on 2026-08-10; this line read "46 of 65", which matched no partition of the array. Recount rather than adjust: `BIOMARKER_SEED.filter(b => b.higherIsBetter === null).length`.)* Most markers are U-shaped — harmful at *both* extremes (ferritin, vitamin D, TSH, sodium, testosterone, IGF-1) — and marking one `1` would have the app telling the user "more is better" about something that hurts them high. A test asserts this for the known U-shaped set.
