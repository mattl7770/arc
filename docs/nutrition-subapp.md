...
- `db/screens-render.test.mjs` **§28**: History's control on a past day, the pending line, no control on a one-meal day; meal-detail `2 eggs (100 g)`; As logged `2 eggs`; the review plate's `2 eggs · P 13g` sub-line, the grams field kept at `100`, ATE and noun control with no OF; a fresh counted dish drawn open with ATE and OF.
