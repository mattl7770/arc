...
| **Nothing tested that the hook's `replace` closes the offer** | **fixed**: the step is `replaceDraft` (`review-undo.ts`), driven in §70 with an answer that keeps every key, and pinned by its body in screens-render §25 |
