...
4. **Browse is a Data-pushed hub** (`/knowledge`, the `labs.tsx` model): keyword search, "Your entries" first, "ARC reference" grouped by topic beneath *(0044 split the first run in two — **Personal**, then **Scientific**, then the pack; §2b. **C14 put Coach memory above all three**, so the hub reads memory → personal → scientific → pack, in order of how eagerly the Coach reads them; §2c)*; a serif-prose reader with a provenance footer; soft archive with restore.
...
7. **One new Coach tool: `save_knowledge_entry`** (confirmation-gated write). No new read tool — `search_history` already covers keyword recall, and the tool catalog is ~66–72% of the cached prefix. The registry goes **42 → 43 in one batched change** — one prompt-cache invalidation, not several. *(C14 → 44: `save_knowledge_entry` takes an optional `id` and rewrites, and `retire_knowledge_entry` archives. Still no read tool; what the read path was missing was the row **id**, which `search_history` now carries. §2c.)*
8. **Memory vs knowledge, the sharp line:** `coach_memories` (0030) = one-sentence durable facts *about you*, injected every turn; `knowledge_entries` = documents, retrieved on demand and cited. ~~Litmus: "true about the user" vs "true about the world / the approach."~~ **Re-cut by 0044 (§2b): the litmus is LENGTH, not subject.** The knowledge base now has a personal section, so both stores hold facts about the user; what separates them is that a memory is one line carried in every prompt and an entry is a page retrieved when relevant. *(C14 put the two stores on ONE SCREEN and did not merge them — the line got sharper, not blurrier, because it is now stated where the owner writes: on the memory run and again in its editor. §2c.)*
...
- **`search_knowledge`** (semantic) is fully written and deliberately **unregistered** — re-examined at C14 and left that way, because `embedText` is still a hardcoded null and a registered call could only answer `available: false`. `db/coach-memory.test.mjs` §7 now pins the *reason* (`embedderStatus()`), so the suite says to re-register it the day the embedder ships. `ingestMemory` (`src/lib/rag/memory.ts`) stores content vector-less and replaces by origin — the exact pattern entry chunking reuses.
...
**Are the user's two lists nearly-identical with different rules?** No, and deliberately: same subject matter, opposite write paths. The place the ambiguity *does* bite is the model, which has two write tools for "the user just told me something personal" — and that is resolved in the one place it can be, the cached prompt bullet, with the length rule stated explicitly and pinned by `db/coach-tools.test.mjs` §36.

*(Until C14 this paragraph also argued the two lists "are not adjacent" — memory under Settings, entries under Data. That half is now wrong: they are adjacent on purpose. See §2c.)*

### §2c — Memory moved here, and did not merge (C14, 2026-09-14)

Owner: *"The coach shouldn't be reading every scientific article put in there every turn, but I should be able to manually add stuff to coach memory that it should know every turn. Furthermore, coach needs to be able to interact with both of these on a read and write basis."*

That is **relocate, not merge**, and it leaves everything above intact: `coach_memories` is still its own table, still capped at `MEMORY_PROMPT_LIMIT`, still injected verbatim every turn. **No migration** (head stays `0045`). What moved is the SURFACE.

**What the hub shows now**, top to bottom: the *Add to the base* stamp, the search field, then four runs — **Coach memory**, **Personal**, **Scientific**, **ARC reference**, with **Archived** at the foot. They are ordered by how eagerly the Coach reads them — every turn, then when relevant, then last — which is also the order of how much a row costs and how much it matters, so one ordering serves both.

The memory run draws one-line rows under a kind eyebrow, a *Remember something* ghost action, and its own collapsed **Forgotten** foot (kept separate from Archived: two stores, two words for the same act, and one heading over both would have to pick a word that lies about half its rows). The search field filters all three writable stores through the shared `queryTerms` splitter and the same distinct-term ranking, so "matches" means one thing on this screen; the two feet are deliberately NOT filtered, because a search over things that cannot be found is theatre.

**The cap is stated in words, and only when it bites.** Under 40 the sentence is a setting nobody asked about; over it, it is the only explanation anywhere in the app for a Coach that ignores a fact the owner can still read on screen — so past the limit the run says how many have stopped riding along and what to do about it.

**The route decision: `app/coach-memory.tsx` was KEPT and repurposed** as the pushed editor for ONE memory — no `id` to write a new one, an `id` to edit, forget, restore or delete. The list lives on the hub; this writes. Keeping it (rather than deleting the route and inlining an editor) is what guarantees there is no second list to drift, and the file is where the LENGTH litmus is stated beside the field, at the moment the owner is choosing between the two stores. **Settings › Coach memory keeps its row and its tally, and the row is now a link to `/knowledge`** — not a duplicate list.

**Read and write on both stores, audited.** `coach_memories`: read by `get_memories` and `search_history`, written by `remember`, retired by `forget` — complete before C14, and left alone. A model correcting a memory is `forget` then `remember` (two gates on the store that rides in every prompt, and no schema tokens); `updateMemory` exists for the owner's editor only. `knowledge_entries`: read by `search_history`, written by `save_knowledge_entry`, and now **rewritten** by the same tool given an `id` and **retired** by `retire_knowledge_entry`. The missing piece was never a tool, though — it was the **id**: a search hit was a dead end, so the Coach could read an entry and had no way to name it again. `searchUserHistory` now carries the row id on the two hit kinds the Coach can write back to (a user entry, a memory) and deliberately not on the pack, which is not the owner's to revise.

**`search_knowledge` stays unregistered**, re-examined and unchanged. It is the tool that names the knowledge base, so registering it looks like the direct answer to "the Coach needs read access" — but `embedText` is still a hardcoded null, so every call would return `available: false` while costing ~180 tokens of the tool ceiling to advertise a dead end. `db/coach-memory.test.mjs` §7 now asserts the *reason* (`embedderStatus() === 'model_not_installed'`) rather than the fact, so the day the embedder ships the suite says to re-register it.

**Budget:** 9,223 → **9,218** tok over 44 schemas (ceiling 9,250, untouched) — the feature landed *below* where it started. Paid by four trims, three of them descriptions reciting their own schema (`remember`'s category enum, `get_metric_series.metric`, `log_screening_done.id`) and one a description restating what its payload already says at runtime (`get_screenings`' empty-ledger sentence). `forget` lost a sentence it had to lose anyway: it told the model the user could still see a forgotten memory *in Settings*. The accounting is in `db/coach-eval.test.mjs` §6.
...
| Route | Screen |
| --- | --- |
| `app/knowledge.tsx` | The hub (`StackHeader` back to Data). **Since C14 the first run is Coach memory** — one-line rows under a kind eyebrow, tapping into `/coach-memory`, a *Remember something* ghost action, its own collapsed **Forgotten** foot, and the prompt cap spelled out in words once it bites (§2c). Then: a keyword **search field** filtering all three writable stores in place — `queryTerms` + OR-of-LIKEs over title/topic/body, ranked by distinct-term count; deliberately *not* `searchUserHistory`, which is the Coach's cross-source tool. **Your entries** first (active, newest-updated; row = serif title · topic eyebrow · muted source line — "written by you" / "imported · outlive.com" / "saved from a Coach conversation"), then **ARC reference** grouped by topic (ruled plates per topic, the labs category-plate model). One stamped plate spends the accent: **Import an article** → `/knowledge-import`; **Write an entry** rides beside it as a ghost action (⚑ MATT #3 to flip). Collapsed **Archived** section at the foot (restore / delete — `coach-memory.tsx` verbatim). |
...
| `app/coach-memory.tsx` | **The memory editor** (C14, §2c) — one memory, not a list. Pushed from the hub's first run: no `id` writes a new one, an `id` edits it and offers **Forget / Restore** and **Delete**. A form, no device, the `knowledge-entry-edit.tsx` model verbatim: content field, neutral kind chips, one accent on Save. The LENGTH litmus sits under the field, where the choice between the two stores is actually being made. It was the whole memory surface under Settings before C14; the list moved to the hub and the route stayed, so there is exactly one list. |
...
The owner's *"Open memories and knowledge only."* The Coach's `delete_record` reaches `knowledge` and `memories` through `deleteKnowledgeEntry` and `deleteMemory`, the functions the Archived list and the memory screen call. Retiring (`status: archived`) stays the ordinary, restorable answer; the delete is for an entry the owner wants gone. **It takes two acts, as on the screen:** the Delete lives on the Archived list only, so the Coach refuses an entry still in every search and names the archive; the permanent delete is for an archived one, and an entry restored while the card was open refuses too. The card names the section, the topic, the day it was saved, the day it was archived, and its opening words — the title alone does not say what a page holds (`docs/coach-domains.md` §10b). The shipped pack is still unaddressable: its chunks have no entry id. The Archived list's own delete line now ends *"Archived entries can be restored. Deleted ones can’t."* (was *"Restore exists; undelete does not."* — an owner-approved cut, `docs/ai-slop-candidates-2026-09.md` §10.G).
