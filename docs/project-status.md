...
- [~] **Protocols** — the versioned stack/routine **editor is built** (2026-07-26, parallel build): `app/protocols.tsx` (list, pushed from Data's "The full file") + `app/protocol-edit.tsx` (create/edit; every content save is a new immutable `protocol_versions` row, no-op saves skipped). Repo `src/lib/db/repositories/protocols.ts` over the 0001 tables (no new migration); tests `db/protocols.test.mjs` (38). **Still to come: the protocol→mission generator** (active protocols → the day's `log_entries` on Home — flagged seam in the repo header) and the Coach's protocol tool.
