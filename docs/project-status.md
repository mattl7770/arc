...
| Modes | 🚧 | **Engine + Coach control built (2026-07-31):** `day_modes` (0026, single-day/range/open-ended), the mode registry (Normal/Travel/Sick/Deload/Social/Custom — drop-types + injected items + heroFocus + coachTone + excusesSkips), the **mode-aware mission generator** (Sick drops training + injects rest/fluids/immune; Social/Travel inject guardrails), the real **`set_mode`** Coach tool + mode surfaced in `get_today_snapshot` + doctrine. Fully functional via the Coach. **Remaining (device-phase UI):** the Home quick mode-control + persistent indicator, and mid-day mission re-derive (deferred: completed-item edge cases). Headless: modes 20 |
...
| Experiments | 🚧 | **n-of-1 loop built (2026-07-31):** `experiments` (0027), the repository (create with computed end-date, active/ready/daysLeft, complete/abandon), and the real Coach tools — **`create_experiment`** (one intervention + metrics + duration), **`get_experiments`** (active + `ready`-to-read-out + completed), **`complete_experiment`** (verdict + readout) — + doctrine. The Coach designs, tracks, and concludes experiments via existing series reads. **Remaining (device-phase UI):** an experiments list/detail surface. Headless: experiments 15 |
| Tests / CI | 🚧 | No app-level runner/CI yet, but the data layer has headless `node:sqlite` tests — **1,127 across 36 suites** (`npm run db:test`; adds export `db/export.test.mjs` 22 incl. the scalar-schema tripwire + 9 app-lock cases in `db/user.test.mjs`, wearables' health-mapping 79 / wearables 33 / readiness 25, **labs** 107, RAG chunker 13 / rag 19, **modes** 20, and **experiments** 15) plus schema validation (`db:validate`, 20) |
