...
| Data | ✅ | **Frame A "Standing Ledger"** — live trends (Weight/Nutrition/Training/Symptoms sparklines from real data), the 12 seeded biomarker reference ranges, a manage/browse index, + pushed **Labs**, **Protocols**, **Screenings**, and **Wearables** (30-day sparklines per metric, source labels, ingested-workout list) screens. Real lab values still await the PDF pipeline |
| Settings | ✅ | **Real** — Profile (name/DOB/sex/timezone) + Units (lb/kg · oz/ml · in/cm live; °F/°C now consumed by the wearables temperature rows) + **Coach** (Keychain key + model picker) + **Apple Health** (live screen: enable w/ lazy permission, Sync now, honest pre-build + read-opacity states). App-lock / backup / export still disabled "Soon" rows |
...
| Tests / CI | 🚧 | No app-level runner/CI yet, but the data layer has headless `node:sqlite` tests — **908 across 30 suites** (`npm run db:test`; the prior 27 plus wearables' health-mapping 79 / wearables 33 / readiness 25) plus schema validation (`db:validate`, 20) |
...
- ⚠️ **`op-sqlite` is active; SEVEN more native modules are added and one `eas build` activates them all (2026-07-29).** `expo-secure-store` (Coach key → Keychain), `expo-notifications` (reminder nudges), `expo-camera` + `expo-image-manipulator` (nutrition phase-2 barcode/photo), `expo-local-authentication` (app lock — module ahead of the feature), and now **`@kingstinct/react-native-healthkit` + `react-native-nitro-modules`** (Apple Health ingestion, read-only + no background delivery, config plugin set) are all in `package.json` + `app.json`. Run the dev build (`docs/dev-build.md`: `eas build --profile development --platform ios`) to make them live on device; each degrades gracefully (no crash) until then. `expo-doctor` is 19/20 as of 2026-07-29 — the one failure is upstream SDK-57 patch drift on 12 pre-existing expo packages (predates the wearables window; `npx expo install --check` clears it whenever convenient). The RAG embedder (`onnxruntime-react-native`, `docs/rag-embeddings.md`) is deliberately **not** in this build — it's heavy and not next, and gets its own.
- ⚠️ **A migration must never land on a number below `PRAGMA user_version` — the runner skips it SILENTLY.** `pendingMigrations` filters `version > currentVersion`, so once a device has run 0021 a migration merged later as `0019_rag.sql` is never applied: no error, just missing tables and a "no such table" at first use, while a fresh install works fine (it applies 0019 in order). The reserved-gap convention (0019 = RAG, 0024–0026 = labs) is only safe for branches that merge *before* a higher number ships — and 0020/0021 have now shipped past 0019. **The RAG window must renumber its migration to 0022+ when it lands** (0022–0023 are free). Surfaced by the 2026-07-29 adversarial review.
