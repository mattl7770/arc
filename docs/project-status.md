...
| Data export | ✅ | **Built 2026-07-29** — whole-DB JSON (all 31 tables via `sqlite_master`) to Documents + share sheet; headless serializer tests (18). `expo-sharing` rides the next build — until then it saves + surfaces the path |
...
| Tests / CI | 🚧 | No app-level runner/CI yet, but the data layer has headless `node:sqlite` tests — **TESTCOUNT across 32 suites** (`npm run db:test`; adds export `db/export.test.mjs` 22 incl. the scalar-schema tripwire + 9 app-lock cases in `db/user.test.mjs`, wearables' health-mapping 79 / wearables 33 / readiness 25, and **labs** 107) plus schema validation (`db:validate`, 20) |
