...
- ⚠️ **`op-sqlite` is active; FIVE more native modules are added and one `eas build` activates them all (2026-07-27).** `expo-secure-store` (Coach key → Keychain), `expo-notifications` (reminder nudges), `expo-camera` + `expo-image-manipulator` (nutrition phase-2 barcode/photo), and `expo-local-authentication` (app lock — module ahead of the feature) are all in `package.json` + `app.json` (permission strings set) and pass `expo-doctor` 20/20. Run the dev build (`docs/dev-build.md`: `eas build --profile development --platform ios`) to make them live on device; each degrades gracefully (no crash) until then. The RAG embedder (`onnxruntime-react-native`, `docs/rag-embeddings.md`) is deliberately **not** in this build — it's heavy and not next, and gets its own.
