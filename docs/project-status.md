...
- [x] Brief + mission are real (deterministic brief via `useDailyBrief`; protocol-generated mission); [ ] 📋 readiness + metrics strip still mock — land with wearables
...
| Settings | ✅ | **Real** — Profile (name/DOB/sex/timezone) + Units (lb/kg · oz/ml · in/cm live; mi/km · °F/°C stored, not yet consumed) + **Coach** (live: Keychain key + model picker) + **Security & data** (App-lock toggle + Export, 2026-07-29). Apple-Health / backup still disabled "Soon" rows |
...
| Tests / CI | 🚧 | No app-level runner/CI yet, but the data layer has headless `node:sqlite` tests — **802 across 28 suites** (`npm run db:test`, the original 18 plus the exercise suites (catalog, routines, training-engine, programs, training-volume), nutrition suites (foods, nutrition-v2, barcode), mission-generate, and export — `db/export.test.mjs` 22 incl. the scalar-schema tripwire, plus 9 new app-lock cases in `db/user.test.mjs`) plus schema validation (`db:validate`, 20) |
...
- ⚠️ **`op-sqlite` is active; FIVE more native modules are added and one `eas build` activates them all (2026-07-27).** `expo-secure-store` (Coach key → Keychain), `expo-notifications` (reminder nudges), `expo-camera` + `expo-image-manipulator` (nutrition phase-2 barcode/photo), and `expo-local-authentication` (app lock — **feature now built**, 2026-07-29) are all in `package.json` + `app.json` (permission strings set) and pass `expo-doctor` 20/20. **A sixth, `expo-sharing`, was added 2026-07-29** (data export's share sheet — the export itself works without it: `expo-file-system` ships as a dependency of the `expo` package, so writing to Documents is live in the pending build; `expo-file-system` is now also pinned as a direct dep). Run the dev build (`docs/dev-build.md`: `eas build --profile development --platform ios`) to make them live on device; each degrades gracefully (no crash) until then. The RAG embedder (`onnxruntime-react-native`, `docs/rag-embeddings.md`) is deliberately **not** in this build — it's heavy and not next, and gets its own.
