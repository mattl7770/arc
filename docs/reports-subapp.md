...
| 2 | **Adherence ledger** | Per protocol active in the period (via `log_entries.protocol_id`): planned / completed / partial / skipped / **excused** / **unmarked** — excusal per-day via the app's ONE definition, `excusedDatesIn` (2026-09-19: this section used to name `accountForDay` against that day's `day_modes` row, which was a SECOND definition — a timezone-changed day was excused on Home and counted as a plain skip here). Three reasons now: an excusing **status** (0061), a frozen **mode**, a **timezone change**. An untouched `pending` row on a SETTLED excused day is excused too, exactly as Home counts it. **The ledger must sum** (`completed + partial + skipped + excused + unmarked = planned`), asserted in tests and visibly reconciled in the table; completion % over *accountable* items (planned − excused). Every reason named beneath (`excusedNote`): *"3 sick days, 4 traveling days and 1 timezone day — skips on those days are excused, not missed. A day can have more than one reason."* — the last clause because a day may carry two, so the counts are reasons rather than a partition and must not be summed. Empty: "No protocol was active this period." |
| | ↳ **`unmarked` is a sixth column the spec's first draft did not have, and the schema forced it (built 2026-08-12).** `log_entries.status` is `pending \| completed \| skipped \| partial` (0001_init.sql), so a generated mission item the user never touched sits at `pending` forever, and the four-way split had nowhere to put it. Both ways of forcing it into four are fabrications: folding it into `completed` invents adherence, and folding it into `skipped` invents a decision the user never made — which would then be silently **excused** on an excused day, inventing an excuse for it too. So it is its own column, it is in the denominator, and the report says what it is. Deciding not to record something is not the same as deciding not to do it. |
