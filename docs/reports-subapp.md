...
| 10 | **What changed** | Protocol revisions (`protocol_versions` created in-period, with `change_notes`), target changes, the period's mode ledger (historical `day_modes` runs only, since the 2026-08-25 removal). The section that makes this a *review* rather than a dashboard printout. |
