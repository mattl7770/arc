...
> ### Four more owner requests, 2026-08-12 — migration **0035**, folders, and plain-text recipe editing
>
> Built and headless-verified — **recipes 189 · grocery 79 · screens-render 142**, `db:test` 46 suites / **2,352** assertions / 0 failures, `db:validate` 20/20, `tsc` 0, `eslint` 0 errors, `expo export --platform ios` clean — and **none of it has been seen on a device.** Two new routes (`recipe-revise`, `recipe-folders`), so the pushed-route count goes 43 → 45.
>
> **2. Folders in the recipe book** (*"Make functionality for creating folders in the recipe book."*) — migration **0035**: a `recipe_folders` table and a nullable `recipes.folder_id`.
>   - **A filing system, not a tag cloud.** One folder per recipe (a column, **not** a join table): a recipe in three "folders" is not filed anywhere, it is described three times, and "where does this live?" stops having an answer. `recipes.tags` already exists for the describing job — tags describe, folders file.
>   - **Flat, not nested**, deliberately. Nesting buys a path model, breadcrumbs on every screen naming a folder, a move-into-own-descendant cycle check, and an orphan-subtree rule on delete — four mechanisms, each with its own way to be wrong, to organise a cookbook that will hold tens of documents and perhaps a dozen drawers. It stays additive: a nullable `parent_id` is a later migration and nothing on the recipe side changes. A join table would have closed that door; this does not.
>   - **`ON DELETE SET NULL` is the load-bearing word.** Deleting a folder UNFILES its recipes and never deletes one — the armed delete says so in words before the second tap, and `db/recipes.test.mjs` §18 asserts it.
>   - **Unfiled is a place.** Every recipe arrives with `folder_id NULL` (import, the manual editor, the Coach's `save_recipe`), the book's default view is **All**, and Unfiled is a chip beside the drawers rather than a quarantine — a recipe in no folder is never harder to reach than one in a folder.
>   - **Two surfaces, and the split is the point.** `app/recipes.tsx` gets a **filter** strip (All · each drawer · Unfiled · Manage); `app/recipe-folders.tsx` owns create/rename/delete. A filter and a destructive control must never be the same row under the same finger. A recipe's own folder is drawn with its provenance at the top of `recipe-detail` and writes on tap, with no accent — filing is instant and reversible.
>   - Names are UNIQUE on `name_norm` (two drawers labelled "Dinners" is a filing error). Safe where 0034's coupling CHECK was not: the index is created on a table that is empty by construction, and the only ALTER carries no CHECK.
>
> **3. A recipe can be edited in plain English** (*"…i.e. 'change the milk for almond milk and add something to keep the sweetness the same' or 'make it higher protein and lower calorie'"*) — `app/recipe-revise.tsx`, pushed from `Edit in words` beside `Edit`.
>   - **The two examples are two jobs, and both belong to the model.** One is a substitution needing a compensating change nobody named; the other is a GOAL that names no line at all, so deciding what to touch IS the request. Neither is a rule (the coach-judgment-not-rules memory).
>   - **A proposal with a diff, never a silent overwrite.** The model returns the whole revised recipe; `diffRecipeLines` marks every line kept · new · dropped, in the order of the recipe you would end up with, and nothing is written until Save.
>   - **Provenance survives the edit, and this is the load-bearing half.** `applyRecipeRevision` matches lines by exact wording, first-come, and KEEPS the matched rows — so a line the owner priced by hand keeps its food link, its snapshots and its `resolved_by = 'user'`. A reworded line is a different line and lands unresolved, which is right: "whole milk" → "almond milk" must not inherit the milk's numbers. Then the **deterministic catalog pass runs before anything reaches a model**, and only what it cannot reach falls through to the model pass, stamped `estimated`. The review states both halves — including, by name, when a hand pick is about to be reworded.
>   - **One matching rule, three callers.** The review, the "what this costs" disclosure and the write all read `survivorFlags`; a test asserts the review and the write agree, because a screen describing a different write than the one that lands is the failure mode here.
>   - **Cost.** One no-tools turn: the Coach's ~8k-token tool catalog is never uploaded, the system prompt is a module constant so it rides the cached prefix, the recipe goes up as words (no macros, no snapshots, no ids) and the reply is raw lines rather than a per-line object — `parseIngredientLine` derives the overlay for free. **No Coach tool was added**, deliberately: a tool would cost tokens on every turn forever and invalidate the cached prefix, to serve a button that already exists on the recipe.
>   - `recipe-detail`'s once-per-recipe model-pass guard now keys on the SET of unpriced line ids rather than the recipe id — the detail screen stays mounted under a pushed one, so the old key would have left a revision's new lines unpriced until the recipe was closed and reopened.
>
> **6. The grocery aisles sit closer together** (*"Reduce the gap size between categories on the grocery list."*) The page had four gaps (`mt-4`/`mt-5`/`mt-7`/`mt-8`) doing three jobs, and consecutive categories took the same 28pt this app puts between a section and a different KIND of section — so eight aisles read as eight documents. Three named steps now: `RUN_GAP` 16pt (the next run of the same list), `SECTION_GAP` 24pt (a different kind of thing), `BAND_GAP` 32pt (a cart band, the only thing on the sheet with a title and a rule of its own). A category label now sits ~24pt below the row above and ~20pt above its own first row, so it groups downward with its section.
>
> **7. The grocery LINE is editable** (*"Make grocery list items editable."*) There was already an editor and it edited the wrong thing — the line's **members**. The row the owner reads and taps had no editor at all, so fixing a typo meant opening the line and editing an entry inside it. Name, quantity and aisle now belong to the line (`updateGroceryLine`); the entries appear beneath only when there is more than one, because otherwise the line IS the entry.
>   - **Provenance is untouched by construction:** the edit writes `name` / `name_norm` / `qty_text` / `category` and nothing else, so `source`, `recipe_id` and `food_id` come through — a Coach-added item stays Coach-added and "for Chicken Adobo" survives its own line being renamed.
>   - **A rename moves every member**, so the line does not split; renaming onto a name already on the list **merges** the two lines, which is exactly what the consolidated view means and destroys nothing.
>   - **A line quantity is the line's**: it lands on the first entry and the others are cleared, so `mergeQtyTexts` reads back what was typed rather than summing it into what was there. Said in words above the field whenever a line has several entries.
>   - **The category moved up to the line, and that fixed a quiet lie:** `consolidatedOpenList` takes a line's category from its first member, so re-filing the *second* entry of a two-entry line changed a value the screen then ignored — a control that appeared to work and did nothing.
>
> **An adversarial review ran over the diff before it was committed and found five real defects — three of them data-loss paths. All are fixed; four are pinned by tests** (`db/recipes.test.mjs` §19d and the parser section), and the fifth lives in a focus effect a server render cannot reach.
>   - **A revision could have destroyed a hand pick over a leading space.** A stored `raw_text` is never trimmed on insert — import and the Coach write what they were given — and every line the model returns is trimmed on the way in. A padded line could therefore never match its own echo: it would be deleted and re-inserted unresolved, taking `resolved_by = 'user'`, the food link and the per-batch snapshots with it, on a line nobody asked to change. The write's key and the review's `survivorFlags` now both compare trimmed, so the two still describe the same write.
>   - **The grocery line editor could write stale text back over a fresh entry edit.** Its `name`/`qty` state seeded once, and the row's key (`name_norm`) does not change when a member's quantity does — so editing an entry moved `qtyDisplay` while the field still held the old merged string, and the next blur wrote that string onto entry 0 and **nulled every other entry's quantity**. The fields now re-seed when the database moves under them, without clobbering in-flight typing.
>   - **A committed revision could be reported as a failed one.** `catalogResolveRecipe` sat inside the write's `try`, so a pricing failure printed *"Couldn't save that change"* over a change that HAD saved — and since the screen's `before` is frozen at mount, a second Apply would have diffed against a recipe that no longer existed. The pricing pass is now best-effort in its own `try`, which is the honest shape: it is idempotent and the recipe screen re-runs it on every open.
>   - **A scope could outlive its folder.** Scope the book to a drawer, delete that drawer, come back: the book kept querying a dead id under a heading reading "Folder" — and if it was the *last* drawer, the strip collapsed to its no-folders state with **no `All` chip to escape with**. The scope is now reconciled against the folders that exist, on every focus. ⚠️ Fixed but **headless-unverifiable** — it lives in the focus effect, which a server render never runs; this one needs the device pass.
>   - **A duplicate folder name could print SQLite's own sentence at the user.** The pre-check and the INSERT are two statements, so the UNIQUE index is the real guarantee; its raw text (*"UNIQUE constraint failed: recipe_folders.name_norm"*) now becomes a `DuplicateFolderError` rather than reaching a screen that prints `error.message`.
>   - Two minor ones went with them: an abandoned rename draft survived a folder row being collapsed and would have committed on the next open (the row's key now carries its name and open state), and a model reply that omitted `steps` wrote `'[]'` over the recipe's method — an omitted field is an omission, not an order, so it now keeps the current steps, symmetric with the existing `servings` fallback.
...
>   - **Two passes** (`src/lib/recipes/estimate.ts`). The **catalog** pass is deterministic, offline and free: a MASS quantity read off the line (g/kg/oz/lb — mass→mass is arithmetic, never a density guess) plus a CONFIDENT name match, which is exact or leading-phrase, so a bare "rice" never silently becomes "Rice cakes". It runs inside `save_recipe` and on every screen open, and it is idempotent. The **model** pass takes what is left ("2 tbsp butter", "2 cloves garlic") in one turn, once per recipe, writing `food_id` NULL and `micros` NULL because there is no catalog food behind those numbers.
>   - **Neither pass ever overwrites a hand pick.** The food search survives as the *correction* path, opened from any line and phrased as changing what it counts as.
>   - **The honesty gate is unchanged; what changed is that it now usually passes.** `recipeNutrition` gained `estimatedCount`, and every surface that prints the figures prints it — otherwise "complete" would quietly mean two different things.
>   - ⚠️ **A schema finding worth keeping.** The obvious constraint is `CHECK ((resolved_by IS NULL) = (grams IS NULL))`. SQLite **accepts that on an empty table and rejects the whole ALTER on a populated one** — ADD COLUMN validates a cross-column CHECK against existing rows, so it passes in a test fixture and fails on the one database that matters. The CHECK covers the vocabulary only; the repository maintains the pair. Full reasoning in the 0034 header.
>
> **2. A photo per recipe.** `recipes.photo_file_name` — a base NAME, never a path, because iOS re-issues the app container's UUID on every install (the 0033 decision, for the 0033 reason). The file half is `src/lib/media/recipe-photo-store.ts`, over a generic store extracted from the meal one (`photo-file-store.ts`). **No retention sweep:** a meal photo is evidence for one day's estimate and expires in a week; a recipe is a document the owner keeps for years. The reconciliation passes (dangling names, orphan files) still run on every app open.
>
> **3. Log it shows what the portion comes to.** The sheet now lists every ingredient and the macros for the chosen portion, all of it moving on each keystroke — through `scaleRecipeLines`, **the same function `logRecipe` writes with**. The rows on the sheet ARE the meal items that will land, so the ledger reconciles by construction rather than by two implementations agreeing.
>
> **And on the grocery list (§2b):** check-off now **waits 4 seconds** with an Undo in place before the write lands (leaving the screen commits immediately, so the window is a grace period and never a way to lose a tap), and the single "In cart" section is split into **In cart** (checked within 24 h) and **Old carts** — each with a bigger serif title over a rule, and each clearing only its own scope. `clearCheckedItems` is gone: with two sections on screen it had no honest button to sit behind, since a Clear under one heading that emptied both would contradict the heading directly above it.
>
> **And in the Coach (§2d):** `save_recipe` runs the catalog pass and its description now forbids telling the user to link anything; `add_grocery_items` asks for the quantity it already knows; the doctrine prefers `add_recipe_to_grocery_list` once a recipe exists. Both prompt ceilings held — the room came from deleting duplication, and there is ~1 token of headroom left (`db/coach-eval.test.mjs` §6).
...
1. **Recipes are first-class tables** (`recipes` + `recipe_ingredients`, migration **0031** — this line read 0030, the pre-renumbering figure, until 2026-08-12), not protocol content and not an extension of `meal_templates` — a template is *what I ate*; a recipe is *how to cook one batch, with instructions and servings*.
2. **Ingredients keep the raw imported line as source of truth**, with a parsed qty/unit/name overlay and a link to a catalog food — never fuzzy-matched (the labs rule). ⚠️ **That link was the user's explicit act until 2026-08-12; it is now made automatically and the line RECORDS WHO MADE IT** (`resolved_by`: user · catalog · ai). See the 0034 entry above — this line is kept because the rest of §2a still describes the shape correctly, only the actor changed. Nutrition per serving is still computed **only** from priced ingredients, and still says how many of them are estimates.
...
8. **The share sheet lands via a spike-then-decide:** the already-installed **`expo-sharing` 57.0.8 ships an experimental first-party share-INTO-app extension** (receive API + config plugin, in `package.json` and `app.json`, and in the owner’s binary since the 2026-08-25 rebuild) — spike it first; adopt **`expo-share-intent` v8** (SDK 57 support confirmed) only if the first-party path fails the requirement. Either way it's behind the guarded-require seam, and the paste-URL path works before it and remains the fallback forever.
...
| 8 | Video (library pick) | `expo-video-thumbnails` samples N stills across the clip (`src/lib/media/video-frames.ts`), each bounded to a **768 px long edge**, into the SAME turn as one `Image i:`-labelled multi-image message plus a rail that says the audio was not heard. Never fallen into — a deliberate tap, because it is the dearest rung in the app | **BUILT 2026-09-19** (`docs/spikes/video-recipe-import-build.md`); device-inert until the EAS build that carries the package |
| 9 | Nothing worked | paste-caption screen with instructions ("tap the caption → copy") · manual editor | Always available, styled as intentional |

**Rung 8 reads what is WRITTEN, never what is said.** The Messages API takes no audio block and there is no on-device speech recognition in reach (`docs/spikes/video-recipe-import.md` §3c), so an overlay card, a sticker, a burned-in caption or a screen recording's auto-captions all come back — and a quantity the cook only *spoke* does not. It stays null, and the review says so in words over exactly the lines it applies to. That is the same rule that keeps invented numbers out of the labs and the day's totals; here it is doing its job, not failing. The apps the owner watched do this by transcribing audio on a server, which is forbidden twice over (CLAUDE.md §2; the ADR below).
...
**What's deliberately NOT attempted:** downloading the video or audio (unreachable via share sheet, ToS-hostile, the exact App Store 5.2.3 rejection case, and useless without a transcription pipeline). Deglaze-tier multimodal is a server product; ARC's screenshot and video-stills rungs cover the "silent reel, on-screen text" case honestly — **and rung 8 changes nothing about this sentence**: it reads a file the user already saved, downloads nothing and transcribes nothing.
...
**Added 2026-08-12** (the folders + plain-text-editing round):

- `recipes.ts` gains the **0035 filing verbs** — `createFolder` / `renameFolder` (both refusing a duplicate `name_norm` with `DuplicateFolderError`), `deleteFolder` (SET NULL: unfiles, never deletes), `getFolder` / `getFolderByName` / `listFolders` (with per-drawer counts, alphabetical) / `unfiledRecipeCount`, `moveRecipeToFolder(db, id, folderId|null)`, and `listRecipes`' `opts.folder` tri-state (`undefined` = all · `null` = unfiled · an id = that drawer). Plus `applyRecipeRevision`, which matches lines by exact wording first-come so an unchanged line KEEPS its row, its snapshots and its provenance, and writes only title/servings/lines/steps.
- `estimate.ts` becomes the recipe sub-app's **one model seam** and gains the revision half: `RECIPE_REVISION_SYSTEM_PROMPT` (a module constant, so it rides the cached prefix), `buildRecipeRevisionRequest`, `parseRecipeRevision`, `reviseRecipe` (one turn, `tools: []`), `isRecipeRevisionAvailable`, `RecipeRevisionUnavailableError`, and the pure review helpers `diffRecipeLines` / `droppedRecipeLines` — both built on one private `survivorFlags`, the same rule the write uses.
- `grocery.ts` gains `updateGroceryLine` / `removeGroceryLine` over a line's member ids, sharing a non-transactional `applyItemEdit` with `updateGroceryItem` (`db.transaction` is a literal BEGIN/COMMIT on both engines, so nesting one is a SQLite error, not a no-op).
...
| `app/recipe-revise.tsx` | **Added 2026-08-12.** Plain-text recipe editing: the recipe as it stands → an instruction in a well → one no-tools model turn → a **diff** (kept · new · dropped) with what it costs in provenance → Save. Nothing is written until Save; going back writes nothing. |
| `app/recipe-folders.tsx` | **Added 2026-08-12 (0035).** The filing cabinet: create · rename · delete, each drawer with its recipe count, and the standing reminder that deleting a folder unfiles its recipes rather than deleting them. Reached from the book's `Manage` chip and from a recipe's own folder picker. |
...
- **`expo-video-thumbnails` is added for rung 8** (new native dep, 2026-09-19 — `~57.0.2`, the SDK 57 bundle's own pin, **no config-plugin entry and no new permission**: it decodes a file the already-granted photo permission handed over). It is the *cheapest possible* native addition — no entitlement, no App Group, no provisioning regeneration — but it is **not free of a build**: `expo-video-thumbnails` is in no binary the owner has installed, so rung 8 ships built-but-dormant exactly as the screenshot rung did, answering *"Video import needs the next app build"* until the EAS build carries it. ⚠️ **The package line had to land with the code, not after it.** A guarded `require()` of a package that is not in `node_modules` is a Metro **resolve** failure at bundle time, not a runtime throw — so omitting it would have broken `npx expo export`, the standing merge gate, rather than degrading quietly. The precedent is "installed, binary behind", never "absent".
...
**Slice 4 — the share sheet (needed the EAS build, which happened 2026-08-25).**
