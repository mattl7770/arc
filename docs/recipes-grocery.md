...
**Status:** **BUILT 2026-08-08 (Slices 1–3 + the five screens).** Migrations **0030** (recipes) + **0031** (grocery); repositories `recipes.ts` + `grocery.ts`; pure libs `ingredients/extract/import/categories/quantities`; the eight Coach tools (registry 24 → 32, `docs/ai-coach.md` §2f); screens `recipes` · `recipe-detail` · `recipe-edit` · `recipe-import` · `grocery` off the Nutrition hub's Kitchen rows; the §7 ADR recorded in `docs/decisions.md`. Headless: recipes **76** · grocery **46** · recipe-import **56** (incl. the share-payload router + native-intent redirect) · coach-tools §17 · **screens-render 62** (every new screen + the edited hub/meal-detail rendered via react-native-web SSR over the real migrated schema — `db/screens-render.test.mjs`) — `db:test` 40 suites / 1,538 assertions, all green. **Slice 4's CODE is complete too (2026-08-08):** the §8 spike resolved in favor of the first-party `expo-sharing` share-INTO-app path — the share-extension target is configured in `app.json` (Text + WebURL + Image activation; plugin chain verified via `expo config --type prebuild`, which emits the `expo-sharing-extension` target into `appExtensions` for EAS provisioning), `app/+native-intent.ts` routes expo-sharing deliveries to `/recipe-import`, and the guarded payload seam (`src/lib/recipes/incoming-share.ts` + the pure `share-payload.ts` router) consumes URL/text/screenshot shares once — all no-oping on the current binary. `expo-share-intent` stays the recorded fallback if the experimental path fails on device. ⚠️ **Honest caveats:** all verification is headless (typecheck/lint/tests/`expo export`/SSR render/plugin-chain eval) — **never rendered or run on a device**, so look/feel, keyboard, touch, the live share-sheet flow, and the app's own network fetches are unverified; the EAS build that activates the extension + `expo-image-picker` is an owner action (the last attempt failed on provisioning — and the new extension target means credentials must be regenerated, §8's known gotcha). §10's per-slice device obligations are the checklist for that first on-device pass. This document extends `docs/nutrition-subapp.md` (rounds 1–3): **recipe book · grocery list · AI recipe import (Instagram/TikTok/YouTube/websites) · Coach integration across all of it.**
