...
**Status:** **BUILT 2026-08-08 (Slices 1–3 + the five screens).** Migrations **0030** (recipes) + **0031** (grocery); repositories `recipes.ts` + `grocery.ts`; pure libs `ingredients/extract/import/categories/quantities`; the eight Coach tools (registry 24 → 32, `docs/ai-coach.md` §2f); screens `recipes` · `recipe-detail` · `recipe-edit` · `recipe-import` · `grocery` off the Nutrition hub's Kitchen rows; the §7 ADR recorded in `docs/decisions.md`. Headless: recipes **76** · grocery **46** · recipe-import **42** · coach-tools §17 · **screens-render 62** (every new screen + the edited hub/meal-detail rendered via react-native-web SSR over the real migrated schema — `db/screens-render.test.mjs`) — `db:test` 40 suites / 1,527 assertions, all green. ⚠️ **Honest caveats:** all verification is headless (typecheck/lint/tests/`expo export`/SSR render) — **never rendered on a device**, so look/feel, keyboard behavior, and touch interaction are unverified; the Instagram/TikTok/YouTube rungs were verified from desktop fetches (2026-08-08) and must be re-proven from the app's own fetch on device (Wi-Fi + cellular); **Slice 4 (share extension) is NOT built** — the §8 spike waits on the next EAS dev build, and `expo-image-picker` (the screenshot rung) rides that build behind its guarded seam. §10's per-slice device obligations are the checklist for that first on-device pass. This document extends `docs/nutrition-subapp.md` (rounds 1–3): **recipe book · grocery list · AI recipe import (Instagram/TikTok/YouTube/websites) · Coach integration across all of it.**
...
4. **One standing grocery list** (`grocery_items` + a `grocery_name_prefs` memory table, migration **0031**): category sections from a static on-device keyword table, per-item learned re-filing, autocomplete + staples from the user's own history. Check-off is soft — history feeds resurfacing, never deleted.
...
### `0030_recipes.sql` — `recipes`, `recipe_ingredients`, `meals.recipe_id`
...
### `0031_grocery.sql` — `grocery_items`, `grocery_name_prefs`
...
- **Spike first, then decide (the share extension):** `expo-sharing` **57.0.8 is already installed and already in `app.json`'s plugin list** (it rides the next EAS build for the export share sheet), and it ships Expo's experimental first-party share-INTO-app path: `getSharedPayloads` / `getResolvedSharedPayloadsAsync` / `useIncomingShare` plus a complete extension config plugin (App Group, entitlements, extension target — `node_modules/expo-sharing/plugin`). **The zero-new-dependency path gets the first spike** on the next dev build: configure the receive options, route the delivered payload to `/recipe-import?url=…`, verify cold + warm start. Adopt **`expo-share-intent` v8** (third-party, SDK-57-support confirmed, mature cold/warm handling, typed webUrl) **only if the first-party path's experimental status or delivery semantics actually fail the requirement** — and record which way it went in this doc.
- Either way: activation limited to **Text + WebURL** at first; an ARC-specific App Group; routing sends an incoming share to `/recipe-import`; verified **on device** (none of this works in Expo Go or the web preview).
