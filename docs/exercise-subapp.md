...
> **Phase 12 (2026-09-25) — two answers on the decisions page, no migration.** *"Quietly drop a session with nothing typed if you leave within a few seconds of starting"* — a NEW session still exactly as Start left it, left within **10 s**, is dropped on the way out (slot cleared, rest alert cancelled, nothing on Home or the hub); a kill inside the window keeps it. *"Add range chips"* — **1M · 3M · 1Y · All** on exercise detail's Trend, opening on the shortest range that still shows every session the old 24-session view showed. Under the chart, the Train hub row's own direction comes first, the same on every chip, and then the change across the range, naming it, when that is a different figure. **§16** is the whole record.
...
- ~~**An untouched session kept.** Tapping Start and backing straight out now leaves a Session in progress card and a Home row until it is discarded. Whether that reads as honest or as noise is the owner's call on the device.~~ **Answered 2026-09-25: noise, within a few seconds.** A new session left within ten seconds of Start with nothing changed is now dropped quietly (§16.1).
...
| Time ranges (1M / 3M / 1Y) on the chart | — | **built 2026-09-25** at the owner's ask: 1M · 3M · 1Y · All, the hub's direction kept as the first line, and the change across the range as a second line that names it (§16.2). (The first cut's reason for waiting was wrong in its arithmetic: 24 sessions of a twice-weekly lift is twelve weeks, not a year.) |
...
*Since 2026-09-25 the Trend has range chips. Detail's first direction line is still this one, the hub row's figure, on every chip. Under it, when it is a different figure, a second line gives the change across the chosen range and names it. §16.2 has the rule.*
...
## 16. Phase 12 — a Start taken straight back, and range chips on the Trend (2026-09-25)

Two answers from the decisions page, 2026-09-25, each overriding what was built on 2026-09-23: *"Quietly drop a session with nothing typed if you leave within a few seconds of starting"* and *"Add range chips"*. No migration: head stays `0063`, and `DRAFT_VERSION` stays 3.

### 16.1 The quiet drop

**The rule** (`liveSessionQuiet`, `src/lib/exercise/draft.ts`). A **new** session that is still exactly what Start put there, left within **10 seconds** of Start, is thrown away on the way out: its draft slot is cleared, its rest alert cancelled, and neither Home nor the Train hub offers it back. Nothing is asked. Everything else is kept exactly as §14 built it: a session with anything changed, one left after ten seconds, a resumed session, an edit of a stored one.

**Why ten seconds** (`QUIET_LEAVE_MS`). "A few" read literally, with room for the case the rule exists for: a Start tapped by mistake, or tapped to see what a saved workout holds, then the back gesture. That takes two to six seconds, and a slow read of a six-exercise list fits inside ten. Past it the owner has had time to begin (walk to the rack, load the bar), and §14's rule takes over.

**What a wrong drop costs is not bounded by the window** (corrected in review, 2026-09-25; the first cut said it "rounds away"). An untouched session holds nothing Start cannot rebuild except its start instant. But the drop is silent, so he finds out only when he next looks for the session, which can be many minutes later, and a Start pressed then dates the session from then. That is §14.8's loss come back on one narrow path: Start Push Day, swipe back at six seconds to check today's mission, warm up for twelve minutes, open Train, and there is no card. He presses Start again and the session is dated twelve minutes late. The only repair is the start stepper on the new session, five minutes a step, and only if he notices. What keeps the path narrow is the window: the glance has to come inside ten seconds with nothing touched first. This trade is put to him in §16.4, not assumed.

**"Nothing typed" is measured, not tracked** (`liveStartOf` / `liveSessionUntouched`). The screen takes one snapshot when Start builds the session: its blocks, its start, Away, the rest timer. Leaving compares the session with it, serialised whole. So every change the owner can make counts: a value typed into any field, a set ticked, added, removed or made a warmup, an exercise added, removed or reordered, a superset bound, the start moved, Away turned on, a rest running. Comparing the whole serialised state, rather than a list of fields, means a field added to the draft later is covered without anyone remembering it, and the only error the comparison can make is keeping too much (a different key order reads as a change). Something typed and deleted again reads as untouched: the screen is back to what Start put there, so the drop loses nothing he can see. A blank sheet stays quiet until an exercise is added, and with no exercise there is no draft to clear anyway.

**Where it runs.** The logger's `beforeRemove` handler now calls one exported function, `leaveLiveLogger` (`app/workout-live.tsx`), and nothing else. In order: a settled screen (finished, discarded, stale) leaves; a quiet session is dropped, with `stop` first (the write-through is switched off and the rest alert cancelled, so nothing can put the session back) and then the slot cleared through `clearOwnLiveDraft` (`workout-drafts.ts`, the same guard Finish and Discard use: never another session's); otherwise `leaveGuard` decides as before. The window is measured with the clock at the moment of leaving, not the one-second tick. A failed draft write no longer asks when the session is quiet: there is nothing typed to lose.

**What the screen feeds it.** A server render runs no effects, so the suite cannot fire `beforeRemove`. So the screen's inputs go through two things the suite can reach. The Start snapshot is `liveStartFor({ draft, workoutId }, liveState)` (`draft.ts`): a baseline for a new session, none for a resumed draft or a workout id. And `liveState` is one memoised object built from the four session states (blocks, start, Away, rest). The snapshot is taken from it and the way out compares against it, so the two sides cannot be fed different fields. `liveStartFor` is pinned for each way the logger opens. The render suite pins from the source that the initializer is that call and that the leave gets `start` and `liveState` and reads the clock when it runs. Each of the three regressions the review named (a snapshot of `null`, a leave fed an emptied session, a stale rest timer) fails that pin.

**A resumed or edited session is never dropped.** Neither has a Start baseline. Resuming a session is choosing to come back to it, and an edit has a stored copy.

**The free-form logger** (`app/workout-log.tsx`) already behaves this way: it writes a draft only once something is typed, so a session with nothing typed was never kept there. Unchanged.

**A force-quit inside the window keeps the session. Decided, and tested.** A kill runs no code, so the way out is never asked, and the draft Start wrote on mount is what the next launch finds: Home's row, the hub's card and Resume all offer it, start instant intact. Two reasons this is the right answer and not a gap:

1. **Force-quitting is how the owner recovers from a misbehaving screen.** His note of 2026-09-14, the one that built the draft store: *"losing workout information when closing app mid workout, necessary for fixing when app bugs."* A session he force-quit to get out of a stuck screen should be waiting when he comes back.
2. **The slot cannot tell a kill at five seconds from one at five minutes.** An untouched session is written once, at Start, and never again, so there is no later write to date the kill by. The five-minute one must come back (a session started from a saved workout and left while warming up is the case §14.8 fixed), so both do. Making a kill drop it would need a heartbeat write or a deferred first write, and either one reopens §14.8's loss for a session killed in the background while he warms up.

The cost is the one §14 already had: a Start killed within seconds leaves a card and a row until he discards them.

### 16.2 Range chips on the Trend

**The chips.** **1M · 3M · 1Y · All**, a row under the metric chips on exercise detail's Trend, in the same chip vocabulary (label voice, no accent). 1M, 3M and 1Y start on the same calendar date one, three and twelve months back, clamped to the month (a month before 31 March is 28 or 29 February) and including that day (`trendRangeStart`). All has no start, and is cheap: the screen already reads every working set of the movement in one scan. The chips are drawn only when the movement has two sessions of the chosen metric to plot at all; a range holding fewer says so and names the way out ("Nothing to plot in the last month. Choose a longer range.").

**The default: the shortest range that still shows every session the Trend showed before it had ranges** (`defaultTrendRange`). Until today the Trend drew the latest 24 sessions. The default is the first chip, shortest first, holding at least that many (or all of them when there are fewer). So it never hides a session the owner could see yesterday, and never draws more history than it has to: twice a week → 3M, weekly for a year → 1Y, a movement started this month or a daily plank → 1M. With his history as it stands (logging since August) almost every movement opens on 1M or 3M. A fixed chip could not promise both halves: 3M would hide half of a weekly lift's last 24 sessions, and All would draw years of a daily one. A picked range holds when he changes metric; the metric's own default (`defaultTrendMetric`) is unchanged.

**Two direction lines: the hub row's figure first, then the range's** (`trendView`; revised in review the same day). The first cut replaced detail's direction line with the change across the range. The review showed that could point the opposite way to the hub row the owner had just tapped, on the default view with no chip pressed: home sessions 100, 100, 100, 130, 130, 130, 120 read **−8%** on the row and **+20%** on the screen it opened. With his history (8–16 sessions a lift since August) the default range usually holds more than four home sessions, so this would have been the common case. Now:

1. **The first line is the row's own figure**, "−8% on the previous 3 sessions": `trendOf` over `sessionSeriesFrom(rows, metric)`, exactly what `directionOf` computes, in the same words VoiceOver reads on the row. It is the same on every chip. It is drawn when the session it is read from (the latest home session on record) is on the chart; a range holding only away sessions draws no direction line, and its large figure is the away value, marked.
2. **The second line is the change across the range, naming it** (`rangeTrendOf`, `rangeTrendPhraseParts`): the latest home session in the range against the mean of the first up-to-three home sessions in it, level within ±2%. Examples: "+20% on the first 3 sessions of the last month", "+4% on the first session of the last month", "+19% on the first 3 sessions on record". It is left out when its baseline is the first line's own sessions, which is when the range holds exactly the latest home session and the ones the first line compared it with (four home sessions, or all of them when there are fewer). Then it would only repeat the figure. So a new movement with three sessions shows one line, and on a lift with seven sessions 3M can show one line while 1M and 1Y show two.

The two answer different questions (the latest against the last three, the latest against where the range began), and they can point opposite ways. That is why neither replaces the other, and why each says what it compared. Away sessions are neither baseline nor latest in either line, as before. The large figure stays `trendHeadline`'s: the session both lines are read from. The extent line under them (`trendExtent`) adds the year to a date from another year, since a 1Y or All chart reaches back past January.

**The Train hub's arrow is unchanged**: momentum, read for every movement at once from its latest eight sessions in one statement (`trainedExercises`). One edge stays outside the agreement. When five or more of a movement's last eight sessions were away sessions, the hub's baseline holds fewer home sessions than detail's first line, and the two figures can differ. That was already true before ranges.

**Every figure is computed in `records.ts`.** `trendView` returns everything the Trend draws: the metric and range (picked or default), the chips, the points, both direction lines with their phrases and spoken forms, the extent line, the empty note and the away key. The screen's Trend is `TrendField`, a hook-free named export of `app/exercise-detail.tsx` that only draws the view (the `RoutineLinesSection` precedent), so the render suite draws it on every range and presses each chip through the element tree. The extent line was the one figure the screen used to compute itself; it moved too.

### 16.3 Tests

- `db/exercise.test.mjs` §15: the window's edges (0 s, 4 s, 9.999 s dropped; 10 s and later kept; a clock set back kept); no baseline never drops; `liveStartFor` giving a new session the baseline and a resumed draft or a workout id none; sixteen kinds of change each keep a session left at 3 s; typed-and-cleared, ticked-and-unticked and moved-and-moved-back read as untouched; a blank sheet; `clearOwnLiveDraft` on its own session, another's and an empty slot; a kill inside the window survives a close-and-reopen of the database file with its start instant, and the logger that reopens it takes no baseline.
- `db/training-engine.test.mjs` §14: range starts and their clamps; a range includes its first day; the default for five training patterns, and in each that it shows every session the old view showed and no shorter chip would; `rangeTrendOf` (the first three, one figure with `trendOf` over the same four sessions or fewer, away sessions, too few); the phrases, spoken and marked; `trendView` over logged sets for each chip (the hub's +6% first on every chip, the second line +4% · none · +14% · +19%); the first line equal to `directionOf` on every chip; the review's case (−8% first, then +20% on the month); a range of only away sessions (no line, the away figure marked) and one home session bringing the first line back; a picked range across a metric change; the empty notes; no chips under two sessions; the extent line.
- `db/screens-render.test.mjs` §28: `leaveLiveLogger` over the real slot with baselines taken through `liveStartFor`: dropped at 4 s (nothing asked, `stop` before the clear, no Home row, no hub card), kept when typed, kept at 12 s, kept after a kill (Home's row, the hub's card and the resumed logger all offer it, and leaving the resumed screen at once keeps it), and the other outcomes unchanged. Also the source pin on what the logger feeds all of that. Exercise detail with the four chips, opening on All, the hub's line above the range's. `TrendField` drawn on each range (both lines or, on 3M, one; its session count; only its own chip selected), and each chip pressed. A range with nothing in it. The review's case drawn through the hub row ("down 8 percent on the previous 3 sessions.") and the detail screen it opens (the same words first, then "up 20 percent on the first 3 sessions of the last month"). §9's three direction assertions read the hub's words again and refute the repeated range line.

### 16.4 What only a device can settle

- **Whether ten seconds is right, knowing what a wrong drop costs.** A mistaken Start should never leave a card. But a real start can now be lost on one path: Start a saved workout, swipe back inside ten seconds to glance at Home, warm up, then open Train to no card. The session is dated from the second Start unless he moves it back with the stepper (§16.1). Is that acceptable, or should the window be shorter, or should a Start from a saved workout never drop? A session left at eleven seconds with nothing typed is kept, and still costs one Discard.
- **The swipe back inside the window**: the screen should go with no alert, and the hub should show no card when it comes into view.
- **Two chip rows** on the Trend at 375 pt: whether the range row reads as a different control from the metric row above it, or needs to move under the chart.
- **Two direction lines under the chart.** Whether "−8% on the previous 3 sessions" over "+20% on the first 3 sessions of the last month" reads as two answers or as a contradiction. Whether the second line appearing on some chips and not others (it is left out when it would repeat the first) reads as deliberate. The second line wraps to two lines at 375 pt in 12 pt serif.
- **A 1Y or All chart of a frequent movement** draws its sessions averaged into about 35 bars (the Sparkline's legibility floor), so the latest bar is an average of the last few sessions; the figure above it is the latest session exactly.

### 16.5 The review round (2026-09-25)

An independent review of the first cut raised three findings. All three were real, and all three are fixed:

- **Major: detail's direction could contradict the row that opened it.** Detail's only line had become the change across the range, and on the default view it could point the opposite way to the hub row (−8% on the row, +20% on the screen). Fixed as §16.2 describes: the row's figure is detail's first line on every chip, and the range's change is a second line naming its range. The `directionOf` docblock had kept its old "can never disagree" claim over the changed behaviour. It now says why they agree and names the one edge (away-heavy last eight sessions) where they can still differ.
- **Minor: the cost of a wrong drop was understated.** The rationale said a wrong drop "rounds away". It does not: the drop is silent, and a Start pressed minutes later dates the session from then. Corrected in `QUIET_LEAVE_MS` and §16.1, and put to the owner in §16.4.
- **Minor: nothing tested what the screen feeds the drop.** Every test built its own baseline. Fixed with `liveStartFor`, the one `liveState` object, and the source pin (§16.1, *What the screen feeds it*). Each of the review's three example regressions now fails a test.
