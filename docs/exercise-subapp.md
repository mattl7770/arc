...
| Time ranges (1M / 3M / 1Y) on the chart | — | **built 2026-09-25** at the owner's ask: 1M · 3M · 1Y · All, and a direction across the range that names it (§16.2). (The first cut's reason for waiting was wrong in its arithmetic: 24 sessions of a twice-weekly lift is twelve weeks, not a year.) |
...
*Since 2026-09-25 exercise detail's direction line is read across the chosen range, not against the previous three sessions, and says so; the Train hub's arrow is unchanged. §16.2 has the rule.*
...
**Why ten seconds** (`QUIET_LEAVE_MS`). "A few" read literally, with room for the case the rule exists for: a Start tapped by mistake, or tapped to see what a saved workout holds, then the back gesture. That takes two to six seconds, and a slow read of a six-exercise list fits inside ten. Past it the owner has had time to begin (walk to the rack, load the bar), and §14's rule takes over. A wrong drop costs little: an untouched session holds nothing Start cannot rebuild except its start instant, and Finish stores the duration in whole minutes, where a start ten seconds late rounds away.
...
**The direction line measures the range and names it** (`rangeTrendOf`, `rangeTrendPhraseParts`). The latest home session in the range against the mean of the first up-to-three home sessions in it, level within ±2%: "+6% on the first 3 sessions of the last 3 months", "+4% on the first session of the last month", "+19% on the first 3 sessions on record". The arithmetic is `trendOf`'s, with the baseline taken from the far end of the range instead of from just behind the latest session, because that is the question a range asks: where has this gone over three months? Away sessions are neither baseline nor latest, as before. The large figure stays `trendHeadline`'s, which is the session the line is read from. The extent line under it (`trendExtent`) adds the year to a date from another year, since a 1Y or All chart reaches back past January.

**The Train hub's arrow keeps its own rule, and the two do not have to agree.** The hub's token is momentum, the latest session against the three before it (`trendOf`), read for every movement at once from its latest eight sessions in one statement (`trainedExercises`); VoiceOver reads it with what it measured ("on the previous 3 sessions"). Detail's line is the change across a range. They are different questions, and each figure says which one it answers. What must agree is the chart, and it still does: detail opens on the hub row's metric. When a range holds four home sessions or fewer, the two baselines are the same sessions and the figures are equal by construction (pinned).

**Every figure is computed in `records.ts`.** `trendView` returns everything the Trend draws: the metric and range (picked or default), the chips, the points, the direction, its phrase and spoken form, the extent line, the empty note and the away key. The screen's Trend is `TrendField`, a hook-free named export of `app/exercise-detail.tsx` that only draws the view (the `RoutineLinesSection` precedent), so the render suite draws it on every range and presses each chip through the element tree. The extent line was the one figure the screen used to compute itself; it moved too.

### 16.3 Tests

- `db/exercise.test.mjs` §15: the window's edges (0 s, 4 s, 9.999 s dropped; 10 s and later kept; a clock set back kept); no baseline never drops; sixteen kinds of change each keep a session left at 3 s; typed-and-cleared, ticked-and-unticked and moved-and-moved-back read as untouched; a blank sheet; `clearOwnLiveDraft` on its own session, another's and an empty slot; a kill inside the window survives a close-and-reopen of the database file with its start instant.
- `db/training-engine.test.mjs` §14: range starts and their clamps; a range includes its first day; the default for five training patterns, and in each that it shows every session the old view showed and no shorter chip would; `rangeTrendOf` (the first three, agreement with the hub's figure at four sessions or fewer, away sessions, too few); the phrases, spoken and marked; `trendView` over logged sets for each chip; the hub keeping its rule while detail opens on its metric; a picked range across a metric change; the empty notes; no chips under two sessions; the extent line.
- `db/screens-render.test.mjs` §28: `leaveLiveLogger` over the real slot — dropped at 4 s (nothing asked, `stop` before the clear, no Home row, no hub card), kept when typed, kept at 12 s, kept after a kill (Home's row, the hub's card and the resumed logger all offer it, and leaving the resumed screen at once keeps it), and the other outcomes unchanged; exercise detail with the four chips, opening on All; `TrendField` drawn on each range (its phrase, its session count, only its own chip selected) and each chip pressed; a range with nothing in it. §9's three existing direction assertions now read the range phrase.

### 16.4 What only a device can settle

- **Whether ten seconds is right.** Long enough that a mistaken Start never leaves a card; short enough that a real session is never lost. A session left at eleven seconds with nothing typed is kept, and still costs one Discard.
- **The swipe back inside the window** — that the screen goes with no alert and the hub shows no card when it comes into view.
...
- **The direction line's length.** "+6% on the first 3 sessions of the last 3 months" is two lines at 375 pt in 12 pt serif.
