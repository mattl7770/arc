...
**Not yet built:** the remaining designed states (data-gappy, first-run); the Home brief is not yet re-toned by mode (the Coach itself is, via `get_today_snapshot`). Mission reading/persistence from `daily_logs`/`log_entries` shipped earlier (`useTodayMission`).
