...
**A row opens.** Its tap is the toggle; the trailing chevron (and a named VoiceOver action) pushes the item sheet, `app/mission-item.tsx`. Since 2026-09-25 the sheet holds **today's actions only** — *Skip today*, *Move today …*, *Remove from today*, *Unsnooze*, *Put back*, *Mark not done* — plus two doors: **Edit this item**, which opens the one protocol editor (`/protocol-edit?id=…&item=…`) with that item expanded and scrolled into view, and *Open ‹protocol›*. *Move today …* was *Move to …* until then; it moves this row on this day, and its name now says so, because the item's own time is set in the editor (docs/spikes/protocol-menus-compact.md, option A, the owner's choice).

**Completing a carried row settles every miss it stands for** (the owner, 2026-09-25: *"An item missed on two days shows up once, carried. When you do it: it settles both missed days (both marked late)."*). One carried row stands for every outstanding miss of its item; ticking it marks each of those days *done late* (`late_on`, plus `late_via` so the undo can find them), exactly as a skip of the same row already settled each of them as skipped. Un-ticking re-opens them all. No day's rate moves: a late completion earns no credit, as before.
...
- **An empty day is stated, not disguised.** When the mission has no items, the hero slot renders `src/components/home/mission-empty.tsx` and the Mission section is omitted entirely — rather than the hero's "Today is handled" (a lie on a fresh install) or a "0 of 0" progress bar over nothing. Two variants: *no active protocols* → "Today has no plan yet" → **Build your first protocol**, which opens the Protocols hub (`/protocols`) — this doc said `/protocol-edit` until 2026-09-25, while the code had opened the hub since the first-run fix of 2026-08-08; *protocols exist but expand to nothing today* → "Your protocols put nothing on today" → **Open your protocols**. It takes over the hero's pine slot, so the screen's one-pine budget is unchanged.

**Status — BUILT 2026-09-19, revised 2026-09-23** (migration `0061`, replacing the mode control that stood here from 2026-08-01; the full record is `docs/information-architecture.md` §Status). Home carries **one** thing. It carried two — the owner's Q5(c) answered as *both* — until his note from the device on 2026-09-23: *"the message is there, but i think it would be better if the status button just changed to say 'Sick' or whatever the currently active status is."*

1. **One small target beside the date — the status door** — in the label voice, opening the five chips in a sheet (`src/components/status/status-control.tsx` → `src/components/status/status-rail.tsx`). It reads `STATUS` when nothing is on and the status itself when something is — `SICK`, `TRAVELING`, or `SICK +1` for several, the count in mono. Filled = on, outlined = off, the vocabulary the mode chip established; the word now says WHICH. That is what keeps a running status visible without opening anything. Five chips inlined on the folio row would be five controls competing with the one action this screen exists for; a door is one. **It is also the Coach tab's door** (since 2026-09-21, owner feedback from the device: *"buttons for the status thing on the coach tab need to be moved and put behind another button"*) — the same component, the same sheet, and since 2026-09-23 the same face: nothing is drawn beside it on either screen.
2. **The mono line above the hero is gone** (2026-09-19 → 2026-09-23). It read `Traveling since Sep 12 — skips excused · readiness baselines exclude 4 status days`, and once the door named the status it was the same fact twice, which CLAUDE.md §5 forbids. What only it said — the age, the excusal, the excluded baseline days, and the escalation to *"no recovery verdict until it ends"* once the exclusion is what stopped Recovery grading — is now the **header of the sheet the door opens**, in mono and in the same words. Tapping the line carried the re-ask to the Coach; that is now the door, then the on-chip. Neither the line nor the mode banner before it ever printed a DIRECTIVE ("Recover: sleep, fluids, rest.") — that hardcoded clinical layer is exactly what the retirement removed.

Home never sends a turn. A tap writes the `day_statuses` row FIRST and then carries the canned prompt to the Coach tab as a seeded `prompt` param — a status Home wrote that no prompt followed would be the old Modes failure under a new name. The write broadcasts (`src/lib/status/store.ts`), so the door re-reads: a sheet presented over Home never costs Home its focus. (The readiness view stopped listening on 2026-09-23 — nothing it draws moves when a status starts or ends today, and the sheet derives its own count when it opens.)

**An open workout — BUILT 2026-09-23** (no migration; `docs/exercise-subapp.md` §14). Leaving the live logger stopped discarding the session (owner: *"confirm in progress workouts not getting cleared, should be same for going to rest of the app"*), so a session can now be left open on purpose as well as by an iOS kill — and Home is where the app lands after one. When the live draft slot holds a session — from its first exercise, typed into or not — **one row sits under the status line**: `Workout in progress · 3 sets done · started 14:02` (no count until a set is stamped), mono and muted, with `RESUME ›` in the label voice. It takes the status line's register and not a device, for the reason that line does: an open session is a fact about now, and CLAUDE.md §5 allows Home one hero. It states the start as a clock time rather than an elapsed count because Home does not tick; a session from an earlier day says `started 2 days ago` instead. 44pt, because it is a door; no accent. Zero height on every day without one. The Train hub's **Session in progress** card stays the full surface (both loggers, resume or discard); the free-form logger's draft is deliberately not on Home — a past session being typed up is not a fact about now.
