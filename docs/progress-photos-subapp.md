...
- Binary without the picker (still reachable on an older dev client): the stamp's button disables with the honest "rides the next app build" sentence (the seam's `unavailable` variant already speaks it).
...
| Stage | What works |
| --- | --- |
| **A binary without the picker/camera/manipulator** (the state until the 2026-08-25 rebuild, and still reachable on an older dev client) | The route renders; the gallery draws any existing rows; the Add-photos stamp is disabled with the authored "rides the next app build" line. Every native require stays guarded (the `photo-library.ts` seam; `db/screens-render.test.mjs` walks the import graph for real). Nothing crashes. |
