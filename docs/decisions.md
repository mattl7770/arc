...
The load-bearing idea is **not the palette** — it is the **surface system**. Every content block is a drafting device whose container encodes what it holds: *plate* (records/ledgers), *field* (a verdict, corner ticks, no enclosure), *margin* (prose), *grid* (metrics, rules between cells only), *well* (capture surfaces), *stamp* (the one next action). A block gets exactly one device and devices never nest — enforced at runtime in `__DEV__` by a context check inside `src/components/ui/block.tsx`, because prose alone was the only guard on a primitive that now has **137 call sites across 58 files** (plate 64 · margin 35 · well 17 · field 9 · grid 7 · stamp 5 — recounted 2026-08-09: `rg -o 'device="(plate|field|margin|grid|well|stamp)"' app src | wc -l` → 137, `rg -l …| wc -l` → 58; this ADR, the Status Board and §3 all read "54 files" until then, while 137 was correct throughout).
