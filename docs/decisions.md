...
> **Amended 2026-09-25 — the owner's answers to the questions this ADR left open**
> (`docs/coach-domains.md` §10b). Three of its lists below are no longer true as written:
>
...
>   tab and the Coach alike. The CANNOT line lost "or deleting"; nothing corrects a capture yet.
> - **The catalog food is no longer ahead of the screens.** *"Add a delete to the food's own
>   screen, so you and the Coach can both do it."* Add food deletes one, with an exact Undo, through
>   the same `deleteFood`. Parity holds both ways.
>
...
2. **`expo-updates` is removed entirely.** It contacted `u.expo.dev` on every cold launch carrying a persistent per-install identifier — unsanctioned third-party egress that broke offline-except-AI, for an OTA capability ARC does not use (zero runtime references). Removed from `app.json` (`updates` + `runtimeVersion` blocks) and `package.json`. (`eas.json`'s `channel` labels were left in place at the time as inert. **Removed 2026-09-21:** eas-cli began halting a `production` build with a prompt offering to install `expo-updates` and configure EAS Update because a channel was named — and the prompt defaults to **Y**, so a bare Enter would have silently reversed this decision mid-build. A label that invites its own undoing is not inert.)
...
**The `ArcBackup` native module (decision 1) — WRITTEN 2026-09-19, still unverified until a build runs.** For the four weeks since, this paragraph described files that did not exist: the seam shipped, the module did not, so `requireOptionalNativeModule` returned `null` and every exclusion in decision 1 was a silent no-op — the health record and the photo directories rode the iCloud device backup exactly as if none of this had been decided. The 2026-08-25 build shipped in that state. The module now exists, owner-approved 2026-09-19 ("yes add the backup module"), as three files under `modules/arc-backup/`: `expo-module.config.json`, `ios/ArcBackupModule.swift`, `ios/ArcBackup.podspec`. It differs from the sketch below in three small ways — no `index.ts` (the seam calls `requireOptionalNativeModule` itself, so an unused re-export would only be a second name to drift), an iOS floor of 16.4 and `swift_version 5.9` to match the Expo SDK 57 modules in `node_modules` rather than the invented 15.1, and a file-exists guard that returns `false` rather than throwing when a `-wal`/`-shm` sidecar is not there yet.

**What is verified, and what a build would prove.** Verified on Windows: `expo-modules-autolinking resolve --platform apple` reports `arc-backup` with pod `ArcBackup`, swift module `ArcBackup` and class `ArcBackupModule` — i.e. the module will be written into the generated `ExpoModulesProvider.swift` and its pod into the Podfile. `db/backup.test.mjs` §8 pins the three names against each other at the source, so a rename on one side fails the suite rather than going quiet. **Not verified: that the podspec compiles.** No CocoaPods and no Xcode exist here. If the podspec is wrong the EAS build fails in `Install pods` (a malformed spec) or in `Run fastlane` with `no such module 'ArcBackup'` from `ExpoModulesProvider.swift` (a pod that installed but produced no Swift module) — a build failure, loud, never a silently inert app. The device check is unchanged from the original: enable iCloud backup, add a weight and a progress photo, and confirm `arc.db` and the photo dirs report `isExcludedFromBackup == true`. If the build fails to compile them, **delete `modules/arc-backup/` and rebuild** — the app returns to the no-op state.

*The original 2026-08-23 sketch, kept for the record. The tree is the spec now.*
