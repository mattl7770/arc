...
**How to apply it:** if you are adding a `border-t` between rows, find the `plate` it belongs to. If there isn't one, you want spacing, not a stroke.

### 1a. **A `plate` encloses a multi-row record. One row, or one paragraph, gets no plate — and neither does an empty state.**

The companion rule to the one above, written down because the owner has now raised *"weird boxes"* **three separate times** and each report was a plate drawn around a single thing.

**Decision:** `<Block device="plate">` is legitimate when it encloses a genuine **multi-row record** — a ledger, a schedule, a list with rows to rule between. It is not a wrapper for one control, one paragraph, or a section's contents in general. Concretely, the two shapes that are always wrong:

- **A plate around a single row.** It draws a border whose only job is to contain one tappable line, which reads as a stray box, not as an object. Cut from `app/exercise.tsx` (Quick log), `app/(tabs)/data.tsx` (Settings), `app/(tabs)/log.tsx` (symptom) and both "Scan another" rows in `app/barcode-scan.tsx` — and from the closing catalog section of `app/food-search.tsx`, which is worth its own sentence because it looked like an edge case and was the **primary** one: that section holds a scan row plus "Create a food", but the scan row only appears once a meal is in progress, and the Nutrition tab pushes `/food-search` with no `mealId` (`app/nutrition.tsx`), so the ordinary "Add food" tap landed on a plate closed around a lone "Create a food". The plate now renders in the genuinely two-row branch only; alone, the row sits bare and unruled.
- **A plate that stays drawn through the empty branch.** With no entries there is no record to close, so the border ends up around a `SectionLabel` and two sentences with nothing tappable inside it. The plate belongs **in the non-empty branch only**; the authored empty sentence sits bare on the sheet under its label. `app/protocols.tsx` and `app/wearables.tsx` are the reference implementations; the pattern was swept through eight more files and, in this pass, `src/components/log/recent-logs.tsx`.

**The counter-argument, and why it loses.** `recent-logs.tsx` had a docstring arguing the plate should hold steady across both states, so the record's *place* is drawn before it has contents. That is coherent and it is still wrong: in this design the device **is a claim about what is inside it**, so drawing one around an absence makes the claim false — and the empty state is what a fresh install opens in, which is exactly when a reader has the least context for deciding whether a box is structure or an artefact. If a future case genuinely needs the plate through empty, it is an exception and it gets recorded here; a comment in the file is not enough, because that is how this disagreed with itself for eight files.

**The three authored message screens, including the sacred one.** The same pass found the two shapes above occurring *together*, in the place it mattered most: `src/components/home/mission-empty.tsx` — what Home renders instead of the hero and the checklist when the day has no plan — put a plate around a section label, one headline, one paragraph and one button, in the empty branch, on the **first screen a fresh install opens**. Its docstring made the identical counter-argument to `recent-logs.tsx`'s, in nearly the same words (*"it stands where a record would stand and states that the record is empty"*), which is the clearest evidence available that this was a genuine fork in the reasoning and not a slip. It loses for the reason above; the copy now sits bare under its label, matching `app/protocols.tsx` and `app/wearables.tsx`. The shape had propagated by citation, so the two siblings went with it: `app/+not-found.tsx` (whose docstring named MissionEmpty as its precedent) and `src/components/ui/error-boundary.tsx`. All three docstrings were **rewritten, not deleted** — each states the argument it used to make and why it lost, because the next person to look at an unboxed message screen will have the same instinct.

The fear underneath all three was the same and is worth naming, since it will recur: that unplated copy is *loose text floating on the sheet*. What answers it is not an edge — it is the paper grid, the section label, the type voices and air, which is what the design already says a section is made of.

**The one exception, named — `FoldSection` in `app/(tabs)/data.tsx`.** A collapsible section wraps its toggle header **and** its children in a single plate, so while it is closed the plate encloses exactly one row: the header. That is the banned shape by the letter of the rule, and it is **kept**. It is recorded here, by name, because this rule now has an exception and the paragraph above says an exception has to be written down rather than left as a comment in a file.

*Why it holds.* The plate is not enclosing a single row; it is enclosing a record that is **temporarily folded shut**, and the header is the fold's own handle — it carries the section's tally in `SectionLabel`'s note slot precisely so the count is true whether the section is open or closed. A closed drawer should still look like a drawer: the border is what says the contents are *here, collapsed*, rather than *absent*. Take it off and expanding the section makes a box appear out of nowhere around content that was already on the page, which is a worse artefact than the box was. Nothing is false about the claim either — the device says "a multi-row record lives inside this", and one does; the reader has just closed it, and the tally beside the label tells them how many rows are waiting. This is exactly the distinction §1a turns on, and it cuts the other way here: an empty state's plate claims contents that **do not exist**, while a folded section's plate claims contents that exist and are hidden *by the reader's own tap*.

*Why it is worth an ADR entry rather than a shrug.* Biomarkers defaults folded (65 rows, and before a lab import, 65 em-dashes), so **the Data tab opens in this shape every single time** — it is not a rare state, it is the tab's resting appearance, and it is the shape most likely to be re-reported as a weird box by someone who has not read this. It is also the shape most likely to be cut by the next conforming sweep, since a mechanical search for "plate containing one row" finds it immediately.

*The boundary, so this is not read as a licence.* This exception is **collapsibility, and only collapsibility**: a plate may enclose a single row when that row is the control that expands it into many, and the section knows how many are inside. It is not precedent for re-plating a single row that is simply a single row, and it is not precedent for holding a plate through an empty branch — a section with *zero* rows folded shut is still an empty state, and the empty state's rule wins. Today `FoldSection` is the only caller of this exception; if a second one appears, it goes in this paragraph.
