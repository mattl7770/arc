...
| **Sub-screens** (pushed) | **Settings** (from the bottom of Data) · **metric keypad** (from numeric tiles) · **the Protocols sub-app** (from Home's mission area *and* from Data) · **the mission item sheet** (`/mission-item`, from a mission row's trailing chevron on Home — today's actions, and *Edit this item* into the protocol editor at that item) · Labs import (from Data) · **the Mission record** (from Data's Mission trend row) · the Nutrition and Exercise sub-app families (food search, meal detail, workout live, routine edit, …) |
...
- **Protocols** — ✅ **graduated to its own sub-app, 2026-08-25** (owner call, with the model rework: `docs/project-status.md` §1 › Data domains). It had been one row inside Data's foldable "The full file" section, three interactions deep for the thing that is the *only* source of Home's mission. Now (since 2026-09-25): `/protocols` (the hub — Running / **Ended** / Paused, each row leading with what the protocol puts on TODAY) → `/protocol-detail` (the working screen: Now with tappable items and *Add an item*, Coming up, adherence, and a closing plate with *Pause* / *Resume* and *Version history*; *Edit* in the header) → `/protocol-edit` (**the one editor**: every protocol fact, create and edit alike, opening at an item with `?item=` or with a new one with `?add=1`) → `/protocol-versions` (the timeline, with a **diff** between adjacent versions and **Restore**). *`/protocol-item` and `/protocol-settings` existed from 2026-09-19 to 2026-09-25; see the compaction below.* **Two ways in, one hub**, like Nutrition and Exercise: a quiet label-voice link in Home's mission area, and Data's existing row. The first-run path from `mission-empty.tsx` lands on the hub rather than on a blank form, and the hub's empty state offers both "build it yourself" and "ask the Coach to draft one" — there is deliberately no template library, because the Coach can read a record a canned "Morning Stack" cannot. **The model behind it**: content schema 2 — ordered phases with durations, and a cadence per item (daily · specific weekdays · every-N-days · an N-per-week flexible quota); migration `0043` adds the phase clock. **An edit reaches TODAY**, through the same re-derive the Coach’s own protocol writes use, so the asymmetry that used to be explained in an 11px margin note is gone. **Since 2026-09-14 (C9/C10/C11, migration `0050`)** an item also carries a **time selector** (a collapsed line that states the time, opening to **the iOS time wheel** and Clear — the wheel since **2026-09-21**, on the owner's device note *"needs a real wheel like a calendar app"*; until then six anchor presets and the app's existing `HH:MM` field, and that typed field is still what draws wherever the picker module cannot load. It writes the same `HH:MM` either way; `src/components/protocols/time-wheel.tsx`) and a per-item **reminder** (off by default, only where there is a time; scheduled through the one OS reconciliation pass in `src/lib/notifications/`, cancelled by the same pass when the item is edited away, ticked, skipped or paused). The protocol ROW carries two toggles of execution **policy** — *If you miss it* (`carry_over`: a missed item is re-offered as a marked row for up to 7 days, never across a phase boundary and never out of an excused day — a status, a frozen mode, or a timezone change) and *When you check it off* (`checkoff_mode`: `strict` keeps the calendar, `adjusting` counts every-N-days from the last completion). A carried row is held out of every adherence denominator (`NOT_CARRIED_SQL`), so the missed day stays a miss and reads **"done late"** on the ledger — the design and its five departures from the spike are in `docs/spikes/protocol-carryover.md`.
  ...
  **The INTERFACE over that model was re-cut on 2026-09-19** (`docs/spikes/protocol-interface-rethink.md`, Phases 0–4). The sub-app had been organised as a **filing system** — documents, each with a reference sheet and one long editor — when what a daily user does with a protocol is *adjust it while running it*. A dose change cost five taps, two scrolls and a guess about which protocol a mission row came from. Five things changed:
  ...
  - **a mission row opens.** A trailing chevron on Home (a 44pt target beside the checkbox, plus a named VoiceOver action on the row; the row's tap is still the toggle) pushes `/mission-item` — what the item is, which protocol and phase put it there, its cadence and next day (or, for a quota, its allowance and no day), and the per-row verbs the repository already had and nothing drew: *Skip today*, *Move to …* (*Move today …* since 2026-09-25), *Remove from today*, *Unsnooze*, *Put back*. Skipping a **carried** row settles the debt behind it too.
  ...
  **The editing surfaces were compacted on 2026-09-25** (`docs/spikes/protocol-menus-compact.md`, **option A — one editor**, the owner's choice). His two device notes: *"theres like so many protocol editing menus now, i think we should compact"* and *"Having two settings menus for protocols is confusing"*. Seven surfaces became five, and there is one protocol-wide form:

  - **`/protocol-item` and `/protocol-settings` are gone.** `/protocol-edit` holds every protocol fact: name, description, type; the items, one line each (time · name · dose · cadence, with ↑ ↓ ×), opening in place to their fields one at a time — the why-line is editable there now, and moving an item to another phase keeps its id; the phases; *Phase 1 starts*, once in the app; the two `0050` policies; the change note; Save; Delete. Create uses the same form.
  - **The protocol page has one door to it**: *Edit* in the header, where *Settings* was. The foot's *Edit* row went. A Now row opens the editor at that item, expanded and scrolled into view, and *Add an item* is drawn at the end of Now whenever a phase is live (until then only on an empty phase).
  - **Pause is not a form field.** It is a *Pause this protocol* / *Resume this protocol* row in the page's closing plate (*The protocol*, with *Version history*), with a confirmation that names what leaves today. One function does it (`src/lib/protocols/pause.ts`: `setActive`, re-derive today, re-sync reminders), and the Coach's `edit_record` on the protocols domain calls the same one.
  - **The item sheet keeps today's actions** and *Edit this item*, which now opens the editor at the item. *Move today …* stays there, named for its scope.
  - **One Save, one transaction, only what changed** (`saveProtocolEdit`). A version is written only when the document or the note changed; a row column only when the form changed it; `is_active` never. A Coach edit approved while the form is open is **merged**, not reverted: the save re-reads the live version and applies the form's changes to it fact by fact (`src/lib/protocols/rebase.ts`), and refuses in one sentence — writing nothing, and reloading the form — only when the same fact was changed two ways.
  - **Completing a carried row settles every miss it stands for**, each marked *done late*, as a skip already settled each as skipped; the undo re-opens them all. No migration: `late_via` is one more key in `log_entries.value`.
...
2. **Quick add — four door tiles and a water row.** The high-frequency structured captures: **Supplement · Weight · Therapy · Screen time** as tiles, and **Water** as its own ruled row beneath them. *(Was 3×2 with two **gateway** tiles — Nutrition and Workout — until 2026-08-12, when the owner cut them: "we don't need the nutrition and workout buttons in the quick log anymore." They could go because Eat and Train became tab roots on 2026-08-09, so this block was offering a second, worse route to two screens the tab bar already reaches. It was then a 2×2 including Water until 2026-09-21 — see *Water* below — then three doors, the last spanning, until Screen time joined on 2026-09-25 and squared the grid again.)*
   - Supplement and Therapy open the capture sheet; **Weight** and **Screen time** open the metric keypad on their own chips. Screen time's door is plan (a) of `docs/spikes/screen-time.md`, "a Screen time tile in Log"; without it the number was reachable only through a grammar he would have to be told.
   ...
   - The block's contract is *every control lands somewhere that writes*; the four tiles open something that writes, and the vessels simply **write**.
   - **Layout.** Two columns is a hard constraint — at three across, "Supplement" truncates, which is why the six-tile grid was abandoned. Four vessels do not fit inside a half-width tile (≈35 pt each, under the 44 pt tap floor), so Water takes a full-width row where four cells are ≈72 pt. With an odd number of doors the last one **spans** rather than sitting beside a hole (a half-empty row reads as a tile that failed to load); with four, as now, the grid is 2 × 2.
3. **Metric keypad (drill-in).** Single-number entry as a calibrated instrument: big mono readout, keypad, and metric chips (**Weight · Water · Body-fat % · Waist · HRV · Resting HR · Dose**). Reached by tapping Weight, or Water's **Other…**; other numbers switch via the chips. *(Tapping Water used to land here; since 2026-09-14 the Log tab logs water itself, and the keypad is the route for an amount that is not one of the three vessels.)* *Wired:* "Log" converts the typed display value to canonical units and writes it — body_metrics (weight/body-fat/waist, in kg/cm), wearable_data (water/HRV/RHR, in ml/…), or a log_entry (dose). **Water** shows additive quick-estimates (**Glass +8 · Bottle +16 · Large +24 oz**) just above the pad, and a live "N oz logged today" line. **Screen time** (2026-09-25, `docs/screen-time.md`) is the eighth chip, reached from its own Quick add door (the chip row scrolls it into view): its `.` key types `h`, Yesterday / Today chips sit in water's slot (before noon it defaults to yesterday), and it writes ONE row per day, replacing the day's number, with the Undo on the Log tab's command field. A day's total is not a capture at a moment, so it is not listed in "Logged today".

**Screen time also arrives by link** — `arc://log/screen-time?minutes=N&date=YYYY-MM-DD` opens `app/log/screen-time.tsx`, which validates and, for calendar today or yesterday over nothing or an earlier Shortcuts number, writes without a tap (a Shortcut can run while the owner sleeps) and offers Undo. An older date, or a day holding a number he typed, gets a confirm card instead. If iOS reclaimed the app, the Undo is on the Log tab's receipt, shown once. Its setup is **Settings › Screen time** (`app/settings-screen-time.tsx`). It reads back on **Data › Trends › Screen time**, the seventh row, which opens the keypad on its chip.
...
Owner request: *"There should be a new screen when you click on the button for Missions in the data tab under trends."* The six Trends rows push `/mission-history`, `/metric-entry`, `/water`, `/nutrition`, `/exercise`, `/symptom` (a seventh, Screen time, joined 2026-09-25 and pushes `/metric-entry` on its own chip). Mission's was the odd one: it ran `router.navigate('/')`, sending the reader to **Home**.
...
- **Layout: a full-width row, and a door that spans.** Four cells do not fit inside a half-width tile — ≈35 pt each, under the 44 pt tap floor — and two rows of two inside it would make one cell of the grid twice the height of its neighbours. At full width the four cells are ≈72 pt. That leaves three door tiles, and three does not divide into two columns: a three-wide row truncates "Supplement" (the reason the six-tile grid was abandoned) and a 2 + 1 leaves a half-empty row. The odd door **spans** instead, so every row is complete and no label is squeezed. A side effect worth having: the tile grid holds one kind of thing again — every tile is a door, and water, which never was one, is its own object under its own label. *(On 2026-09-25 a Screen time door made four, and the grid is 2 × 2 again; the spanning rule stays for an odd count.)*
