...
> **Superseded in part — 2026-09-25.** Two of the surfaces this build added are gone: the per-item editor (`/protocol-item`) and the settings sheet (`/protocol-settings`) folded into ONE protocol editor, `/protocol-edit`, on the owner's choice of option A in `docs/spikes/protocol-menus-compact.md` (his device notes: *"so many protocol editing menus"* and *"two settings menus for protocols is confusing"*). What still stands from here: the mission row's chevron and the item sheet (now today's actions plus *Edit this item*, which opens the editor at the item), the protocol page as the working screen (its header action now reads *Edit*, and pausing is a row on it), *Coming up*, the hub row's lead figure, and the two Coach fixes. Where this document describes `/protocol-item`, `/protocol-settings`, or an editor whose edit path is "structure only", read `protocol-menus-compact.md` instead.
