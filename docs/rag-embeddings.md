...
- **Migration:** one new migration adds a `vec0` virtual table for the vectors plus a companion `knowledge_chunks` / `memory_chunks` table holding the chunk text + metadata (source, topic, date, origin row). **Reserve the next free migration number** at build time — main is at `user_version` 9; exercise holds 0011–0013 and nutrition 0014–0018 once integrated, so the RAG migration lands at **0019+** (confirm the max at integration).
