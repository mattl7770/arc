...
| `delete_record` | write | `{ domain, id }` — remove ONE row. **Its own tool over its own smaller enum (11 of the 26 domains)**, because a removal must never be a *value* the model can set in passing beside three other fields on a card that opens with "Edit". A domain that is a record of a day is absent from the enum entirely, so the schema refuses it at zero round trips, and a `refuse` domain that is asked anyway answers with the AFFORDANCE and the screen ("Pause it with `is_active: false`, or delete it on Protocols"). `hard` is allowed only where no log table CASCADES from the row — asserted against `PRAGMA foreign_key_list`. `own` means **undo**: a meal or a workout goes only when `ai_messages.tool_calls` shows THIS conversation's Coach logged it, checked at card time so the user never answers a gate for a refusal. The card's fixed consequence line finally says a row *leaves* the record. |
