...
> **The registry today: 41 tools — 19 read + 22 write.** `COACH_TOOLS = [...READ_TOOLS, ...WRITE_TOOLS]` (`src/lib/ai/tools/index.ts`) is the single source of truth; this doc is the spec. §2a and §2b below list the original slice (9 read + 10 write); the ones added since are in **§2c** (statuses, experiments, knowledge, nutrition targets, screenings), **§2f** (recipes + grocery, 3 read + 5 write) and **§2g** (the generic tools over the domain registry), and the deliberately withheld ones are in **§2d**. Counted 2026-09-19 by importing the registry, not by adding up this document — the count is 41 rather than 44 because §2g *removed* six tools and added three.
>
...
| `get_today_snapshot` | — | **readiness (`deriveReadiness` — the same verdict Home shows)**, any open **statuses**, mission (`log_entries`, **with item ids**), `meals`, `workouts`, `symptoms`, ad-hoc captures (in the user's units), reminders due today, **running experiments**, **profile (age/sex)** | Today's full picture in one call |
...
**Every turn also opens with a "Current state" system block** (`src/lib/ai/turn-context.ts`, 2026-08-08): date+weekday, profile+units+**goal direction**, any open **status** (only when there is one — no `Mode: Normal` on every turn forever), readiness verdict+pillars, mission progress, running/ready experiments, and the deterministic brief — sent as a second uncached system block after the prompt-cache breakpoint. The model starts oriented and spends tool calls on depth, not on discovering what Home already shows.

**Two settings joined the snapshot and the state block, 2026-09-19** (`get_today_snapshot.goalDirection` / `.waterTarget`; `· goal: <direction>` on the `User:` line). Both are the `nutritionTargets` blind spot again — a shipped, user-editable setting the model was reasoning *around* rather than *from*. `goalDirection` (`cut` | `maintain` | `gain`) is what Home's nutrition pillar grades against, so without it a 600 kcal surplus is unreadable: the plan or the problem. `waterTarget` is reported in the user's own volume unit and is explicitly **`null`** when unset — an unset setting, never a missing feature — because there is deliberately no stock hydration goal. Both cost the schema ceilings **nothing**: they are payload. The goal direction is printed in the state block even at its default, unlike Status and Timezone: those are events, where absence means nothing happened; this is a dial that always has a position.
...
- **The right daily aggregation per metric.** Metrics stored as many rows a day (`workout`, `water_ml`) are **summed**; everything else is **arbitrated** — one winning source per day, richest device first, the same rule Home and the Data tab use, so the three surfaces cannot disagree. **That is `agg`, and it is not the same question as `accumulating`:** `agg` says how many rows fold into one day, `accumulating` says whether that folded day is FINISHED. `steps` is arbitrated (HealthKit merges the day, and two devices must pick a winner rather than double-count) **and** accumulating, which is exactly how a two-hour-old today of 900 steps once got averaged in beside seven complete 8,000-step days. Since 2026-08-25 the second question has one answer for the whole app — `isAccumulatingMetric` (`src/lib/health/accumulating.ts`), derived from the ingest specs — read by `get_metric_series`' `partialDate`, the insight trends, the brief's floor line and the self-review alike.
...
| `set_status` | `label`, `from?`, `until?`, `excuses?`, `note?` | `startStatus` — **and nothing else**. A status records the fact; reshaping today is `adjust_today` on the same turn | "Status: sick today" |
...
Earlier revisions of this doc listed `set_mode` (retired 2026-09-19) and `create_experiment` as unregistered stubs. **Every tool below is registered.**

| Tool | Kind | Notes |
| --- | --- | --- |
| `set_status` | write | Records a standing state the user reports ("sick", "traveling") in `day_statuses` (**0061**), so its days are excused in adherence and left out of the readiness baselines. **It re-derives nothing** — what today should become is the model's call on the turn, through `adjust_today` or `update_protocol`. `excuses: false` records a state that is context without absolution; an OMITTED `excuses` leaves an open status as it is, so a re-ask never silently re-excuses a day. `"normal"` ends every open one. Open-ended is a RAIL affordance only, so the model can never mint a permanently-open row. |
...
| ~~`complete_experiment`~~ · ~~`abandon_experiment`~~ | — | **RETIRED into `edit_record` 2026-09-19** (§2g). Concluding is `{ experiments, status: concluded }` with a `conclusion`; abandoning is `{ status: abandoned }` with a `reason`, and the companions are required in code because a schema cannot say "this field when that value" without splitting the tool in two again. Every guard survives: an unknown id refuses, a concluded experiment cannot be re-concluded, and concluding still re-derives today so a finished intervention leaves the mission. |
...
| `save_knowledge_entry` | write | **Added 2026-08-12 with the knowledge base (0038); gained a REQUIRED `section` at 0044.** Writes ONE entry through the same repository path the editor and the import review screen use, so it is chunked and citable the moment it lands. `source='coach'`. **`section` is `personal`** (a durable fact about the USER, too long for a one-line memory) **or `scientific`** (how something works, or a stance they commit to) — **required rather than defaulted**, because a model that omits it has usually just been told something about the user, and the convenient default would file a fact about his knee in with the articles. The split from `remember` is LENGTH, not subject, and that rule lives once in the cached prompt. Card: `Save <section> entry "<title>" · <topic> · <N> words` — the section is on the card because it decides where the entry can be found afterwards. **The card is compact on purpose and is only safe because of the doctrine beside it:** the model must present the drafted body verbatim in its message *before* calling, so what the user approves is on screen and not just a title. Reach for it only on the user's request or clear invitation — never to file away your own output. **C14 (2026-09-14) gave it an optional `id`: present, it REWRITES that entry whole rather than adding one** — the four content fields stay required, so a revision resends the entry and the user approves the whole new text, and a create still cannot arrive without a body. The card says `Rewrite …` and names the old title when the rewrite renames it, because approving "Save" for something that silently replaces a page you already have is the failure a gate exists to prevent. The repository re-chunks, so a rewrite can never leave stale doctrine retrievable. See `docs/knowledge-subapp.md` §6 and §2c. |
| ~~`retire_knowledge_entry`~~ | — | **Added by C14 (2026-09-14); RETIRED into `edit_record` 2026-09-19** (§2g) as `{ knowledge, status: archived }`. The reversal is narrower than it looks: C14's argument had a CARD half and a SCHEMA half, and only the schema half was about a separate tool. The card half — "taking something out of the base is not a smaller version of putting something in it, and it deserves its own card" — the fold keeps exactly (`Retire entry "<title>"`, `selfEvident: false`, lanes drawn). The schema half was that a `retire: true` **flag on `save_knowledge_entry`** would force `title` and `body` optional, so a create could arrive with no body; `edit_record` is not that flag, because it has **no create path into this domain at all** (`createVia: 'save_knowledge_entry'`). What it did, for the record: archives ONE entry by id — the row keeps its text, its chunks are dropped, so it leaves every search with no `archived_at` join anywhere. Soft: the owner can restore it from the hub's Archived foot. Separate from `save_knowledge_entry` for the reason `forget` is separate from `remember` — taking a page out of the base is not a smaller version of putting one in, and it earns its own card (`Retire entry "<title>"`). The description forbids tidying: retiring is the user's retraction, never housekeeping. Terse schema (`{id}`, 36 tok), the shape `forget` established. |
| `search_knowledge` | read | ⚠️ **WRITTEN BUT DELIBERATELY NOT REGISTERED** — `read-tools.ts` says so in as many words, and this doc claimed the opposite for weeks. Semantic RAG over the corpus via `sqlite-vec` (**0025**), which cannot return a passage until the on-device embedder model ships (§9): a registered tool that always fails teaches the model not to call it. `search_history` covers the same ground by keyword today, over the user's own writing, ARC's pack **and** their own knowledge entries. **Re-examined at C14 and left unregistered**, when the brief was explicitly to give the Coach read access to the knowledge base: the tool that *names* the base looks like the answer and is not one, because it would spend ~180 tok of the ceiling to return `available: false` on every call. `db/coach-memory.test.mjs` §7 now asserts the *reason* — `embedderStatus() === 'model_not_installed'` — so the suite, not a comment, says to re-register it the day the embedder ships, **batched** with whatever else is pending. |
...
Withheld from the model on purpose: a tool that always fails teaches the model not to call it. Each ships when its dependency lands. *(2026-08-08: `create_experiment` and `set_mode` graduated to registered writes — see 2b; the coach-assist wrapper stub was deleted outright. 2026-09-19: `set_mode` was deleted and replaced by `set_status`.)*
...
### 2g. Shipped — the generic tools over the domain registry (`docs/coach-domains.md`)

**Built 2026-09-19**, implementing `docs/spikes/coach-whole-app-access.md` Phases 0–2. The owner's direction was *"basically the entire app should be accessible for reading and writing for the coach"*, and a bespoke tool per gap prices at ~14,000 tokens of cached prefix against a 9,250 ceiling. So the app is described **once in TypeScript** (`src/lib/ai/domains/`, **27 domains** since `nudges` joined on 2026-09-25) and generic tools read that description. `docs/coach-domains.md` is the spec; what belongs here is what the model sees.

Three rails make it safe, and all three are code rather than prompt copy. **Parity with the screens, through the repositories, never past them** — every generic read and write calls the same repository function the matching screen calls; there is no raw SQL and no `run_query`, because a card cannot say what arbitrary SQL will do. **Read-modify-write, always** — a field the patch omits is preserved, which is not a nicety: `replaceWorkout` deletes every set and re-inserts its argument, so a literal patch of a duration would empty a session behind a card that said "45 → 50". **The card is drawn before the gate and re-read after it** — a row that moved while the user was deciding refuses with a named field and a before/after, and no receipt is minted.

| Tool | Kind | What it does, and the rule it carries |
| --- | --- | --- |
| `query_records` | read | `{ domain, id?, query?, from?, to?, limit? }` — read a domain no bespoke tool covers. **Fifteen today**: `meals` (by day, with items), `food_catalog`, `meal_templates`, `micronutrients`, `water`, `captures` (by day), `protocol_adherence`, `exercise_stats`, `exercise_catalog`, `saved_workouts`, `protocol_versions`, `lab_reports`, `knowledge`, `progress_photos`, `reports`. A domain a registered tool already reads is **absent from the enum** — the schema steers rather than a prompt rule — and asking anyway is refused by naming that tool. `list` domains carry an id on every row and cap at 25 (10 by default); `compute` domains return ONE typed object and require an `id`, because adherence and per-exercise records are not rows. **The domain ALONE is the discovery call**: it returns that domain's fields, whether it is editable and what creates there, which is where the field vocabulary lives instead of the cached prefix. **Excluded from the unattended pass** (`PASS_EXCLUDED_TOOLS`), so the pass still carries exactly the eighteen read tools it always did and its prefix moved only with the prompt (7,076 → 7,028, downward). |
| `delete_record` | write | `{ domain, id }` — remove ONE row, permanently. **Its own tool over its own smaller enum (15 of the 26 domains since 2026-09-25)**, because a removal must never be a *value* the model can set in passing beside three other fields on a card that opens with "Edit". **Deletion follows the screens (owner's call, 2026-09-23, superseding the undo-only rule):** every domain in the enum is `hard`, and its `run` is the delete its own screen calls — a meal, a session, a water entry and a protocol included, whoever logged the row. There is no rule about which rows or when; the card is the guardrail. It names the row's day, name and figures (`RemovePolicy.gone`), it is always the long card, and it is re-read past the gate — values and printed line both — so a row that moved refuses. A domain no screen deletes (a protocol version, a lab report) is absent from the enum and, asked anyway, answers with where the row lives; progress photos and reports are held below parity by an owner call (`docs/coach-domains.md` §10a). **Since 2026-09-25** memories and knowledge entries are open (their screens' `deleteMemory` / `deleteKnowledgeEntry`; a knowledge entry once archived, as its Archived list requires) and so are the Log tab's captures, which gained a × of their own (`removeLogCapture`: the row and its Apple Health copy) — §10b. Every CASCADE out of a removable row is asserted to be its own part (items, sets, versions, a knowledge entry's chunks) against `PRAGMA foreign_key_list` over every table. The consequence lane says the row is deleted, with no undo. |
| `edit_record` | write | `{ domain, id, fields }` — patch ONE existing row. **Read-modify-write always**: a field the patch omits is preserved. The card prints before → after, resolved from the row, and throws at card time for anything knowable. `fields` is the registry's **only open schema** (the outer object is still `additionalProperties: false`); the vocabulary is taught by the description, by `query_records`' `fields`, and by an unknown-field error that names the set — never by 26 domains of field names in the cached prefix. Without an `id` it refuses, **naming the bespoke tool** that creates there: prefer-the-specific-tool is a parser rule, not only prompt copy. **Since 2026-09-25** it also corrects an exercise's weight basis (`exercise_catalog` `loadBasis`, the chooser's `setExerciseLoadBasis`) and **combines meals** (`meals` `combine_with: [ids]` + `name`, the Eat tab's `combineMeals`) — the one act its description names beyond the statuses, because no model would guess an edit folds rows together. Past the gate it re-reads the printed line as well as the values, so a combine's other meals are guarded too (`docs/coach-domains.md` §4, §10b). **Also since 2026-09-25** it reaches the notification controls: four `settings` fields (`nudges_enabled`, `quiet_start`, `quiet_end`, `checkin_time`, through Settings › Coach's `saveNudgeSettings`) and a planned nudge's cancel (`nudges`, `status: "cancelled"`, through the Coach tab's `cancelNudge`) — §3 › Coach notifications, and `docs/coach-domains.md` §10c. |

**Six status-change tools retired into it** — `complete_reminder`, `dismiss_reminder`, `complete_experiment`, `abandon_experiment`, `forget`, `retire_knowledge_entry`. Each executed a single `UPDATE … SET status`; together they cost **970 tokens**, and `edit_record` costs 216, which is what funds the registry with **no ceiling raise** (`db/coach-eval.test.mjs` §6). Their **cards are byte-identical** and their receipts keep their verbs; the chip row is the one loss — all six now read "edit record", because `humanizeToolName` runs on the tool name. `RETIRED_WRITE_NAMES` (`src/lib/ai/tools/index.ts`) keeps a persisted call naming one of them classified as a write, so no historical receipt is lost.

**Two behavioural rails moved from those descriptions into read PAYLOADS**, because a generic tool's description cannot carry domain prose and the prompt has single-digit headroom: `list_reminders` emits *"a recurring reminder is never completed…"* when the device actually has a recurring reminder, and `get_experiments` emits *"an experiment with no adherence behind it is abandoned (with a reason), never concluded"* when one is running. Both reads always precede the write — the id comes from them — and both rails are also card-time refusals, so a model that skips the read meets a throw rather than an approved write. Result fields are not in the ceiling budget; descriptions are.
...
**It waits for the Health sync first (2026-09-25).** On a foreground the Health sync and the pass both start from AppState listeners, and nothing ordered them: the pass built its state block at once, usually before last night's sleep and HRV had landed. `maybeRun` now awaits `waitForHealthSyncIdle` (`src/lib/health/sync.ts`) before it decides anything — it yields one macrotask so listener order cannot decide whether it waits, then waits for every tracked Health pass (a queued follow-up included) to settle, for at most 10 seconds, so an unlinked or slow HealthKit never holds the Coach back.

### Coach notifications — the Coach plans its own (built 2026-09-25, migration 0064)

The plan is `docs/spikes/coach-notifications.md`; the owner accepted its six recommendations on 2026-09-25. ARC has no server, so these are **local** notifications, and the Coach can only decide one **while ARC is open**. Nothing here runs in the background (option (d) was rejected), and there is no Reminders screen yet (Q6).

**The model decides; code only parses, caps, de-duplicates, applies quiet hours and schedules.**

- **Where the instructions live.** Any pass — daily, evening, signal, a tapped check-in — is told, in `passDirective`, that it may end its reply with lines `NUDGE YYYY-MM-DD HH:MM text`. That is the pass's USER message: outside both chat ceilings (`db/coach-eval.test.mjs` §6) and outside the cached prefix, so the chat Coach pays nothing for it. With nudges off the directive is word for word what it was before 0064. No tool was added; the pass stays read-only by construction.
- **Parsing** (`parseNudgeReply`, `src/lib/notifications/nudge-plan.ts`). Every line whose first word is `NUDGE` (capitals; bullets, a list number, quote marks and emphasis tolerated) is taken OUT of the reply whether it parses or not — the `isPassSkip` lesson: a sentinel that leaks reaches the owner as the Coach's own words. The scaffolding a model wraps them in goes with them: a code fence that opens or closes the block (fences are paired, so a code block in the note keeps both of its own), a horizontal rule touching it, and a short label or heading directly above it (`Notifications:`, `**Planned nudges**`). Left behind, any of that sat below a `SKIP` and made the reply read as spoken — the 2026-08-11 defect by a new route, found in review. What remains decides spoke vs silent, so `SKIP` plus a nudge line is a silent pass that planned one; and because the directive puts the lines LAST, the verdict is also read where they begin, so `SKIP`, the lines, then a remark about them is still silent. A day in words, an impossible date or a 25th hour is malformed and dropped; the day is never inferred.
- **The caps** (`planNudges`) — each refusal drops the line, never moves it: at most **2 per logical day**, counting nudges already sent that day (Q1); **no number** in the text (Q3: the line shows in full on the lock screen, numbers kept out) — a figure standing on its own is refused in code, while a digit inside a name that starts with a letter (`B12`, `D3`, `CoQ10`, `Omega-3`) passes; at most **140 characters**; at least **5 minutes** ahead and at most **36 hours**, except that a line restating a pending nudge exactly only has to be still ahead (the directive tells the model to repeat what it wants kept, so refusing the repeat of a nudge due in four minutes cancelled it just before it landed); nothing inside **quiet hours** (Q2, default 21:30–07:00); no second nudge in the same slot, and no repeat of words already sent that day. When a pass writes three for one day, the two it wrote first are kept.
- **What the pass is told.** The wall clock (`It is now 13:10 on 2026-09-25.`), so it knows which of today's moments are still ahead; the first build gave it only the date, and code then dropped moments the model could not have known were past. What is pending, and what already went out today. And every line code refused from the LAST pass, with the rule that refused it (`coachNudgeRefusals` in `users.preferences`, replaced by each pass and cleared by one with nothing refused), because a refusal is otherwise silent and the model would write the same line again. All of it is the pass's own user message, so it costs 0 tokens against either ceiling.
- **Rows** (`coach_nudges`, `src/lib/db/repositories/coach-nudges.ts`). A logical `day` plus a wall-clock `time`, so a trip re-anchors the fire instant through `fireInstant` exactly as it does a protocol item. `pending` / `delivered` (he tapped it) / `cancelled`.
- **The thread records it** (Q1). After a pass changes the plan, `Planned notifications` and one line per pending nudge (`tomorrow, 07:30 · Leg day. Eat before you lift.`) are appended — to the Coach's note when it spoke, on their own when it was silent. Home's "Coach noticed" card shows the note only. The record is code-authored and claims nothing about delivery.
- **Delivery.** What will buzz is `upcomingNudges`: pending, still ahead, nudges on, outside the CURRENT quiet hours — the one list the OS resync schedules, the Coach tab lists and the pass is told. `syncReminderNotifications` — the one pass that owns the OS schedule — gained two sources: the pending nudges still ahead (one dated notification each, title "Coach", `data: { kind: 'nudge', nudgeId }`, `interruptionLevel: 'active'` so a future Time Sensitive entitlement cannot quietly let the Coach break through Focus) and the morning check-in below. Quiet hours bind the Coach's nudges only; his reminders, the check-in and protocol items keep his times.

**Decisions this build had to take (the plan left them open).**

1. **A silent pass does not cancel pending nudges.** A pass that writes no nudge line leaves what is pending alone — Haiku forgetting to restate the evening's plan for the morning must not delete it. A pass whose lines produce at least one ACCEPTED nudge replaces every nudge still ahead with its set (a restated one keeps its row, so repeating the plan records nothing). A pass whose lines are ALL refused changes nothing. `NUDGE NONE` is how the model cancels on purpose. A nudge whose moment has passed went out and is never cancelled.
2. **What a tap shows.** A **nudge**: its line is added to the thread as the Coach's latest message, once, and the row is marked delivered so the next pass is told it went out. A **check-in reminder** (Q5): the Coach speaks first, about that topic, once per check-in per day; the row is marked on the Coach tab with **Talk about this**. A **plain reminder**: the row is marked and **Talk about this** seeds the composer with it — never sends. The **morning check-in**: if no pass has run today, the check-in IS the day's pass; if the day's note already exists, the thread shows it and nothing is paid twice; if the day's pass was silent, he is still asking, so the check-in runs once. **When a check-in gets no answer** — the Coach looked and had nothing, could not be reached, or has no key — one plain line above the composer says which, instead of a tap that opened onto nothing (`src/components/coach/checkin-line.tsx`).
3. **Numbers are kept off the lock screen by code**, not only by the directive. The owner accepted a directive-level rule; code backs it up for the case that matters, a figure on its own (`HRV 38`, `7 hours`, `500 ml`, `7:30`). The first build refused every digit, which silently vetoed `B12` and `Omega-3`; a digit inside a letter-led name now passes. `Zone 2` is still refused, because the 2 stands alone and telling it from `Sleep 6` would be judgment. The directive says so.
4. **No `about` link to a mission item (deferred).** The plan's optional link needs a stable identity the pass can name, and the state block lists no mission ids — tomorrow's rows do not exist yet. A link guessed from a title would be code guessing. The directive tells the model to write lines that still hold if the thing is already done.
5. **The evening pass exists only while nudges are on** (it was bought to plan them, Q4). It is the first open at or after 18:00 — or in the small hours before a late day boundary — and on a day whose first open is that late it is also the day's look, so it costs one call, not two.

**The 64 cap, guarded for the first time.** iOS keeps the 64 soonest pending notifications and drops the rest silently. Every sync now fits `NOTIFICATION_BUDGET` = 60 (four spare for the rest timer and whatever comes next): his reminders first, then the morning check-in, then protocol items and Coach nudges merged soonest-first — iOS's own order. What does not fit is counted in `overBudget`, never handed to iOS to drop.

**Taps are routed once, and land where their answer is.** A cold start can deliver the same response twice, and the launching response survives a JS reload. `registerNotificationRouting` remembers each DELIVERY (`tapKey`: the request identifier plus the moment it fired) and clears the launching response once handled. The identifier alone was the first version, and it swallowed tomorrow's tap of a repeating trigger — the morning check-in, a daily check-in reminder — which keeps one identifier across firings until the next resync. The nudge and check-in paths are idempotent underneath as well. On the Coach tab (`coachTapParams` / `coachTapLanding`), a plain reminder goes to the top, where its row is. A check-in reminder, the morning check-in and a nudge go to the END, where the Coach's answer or the nudge's line lands, and the view follows the end until he drags it. The params are dropped once taken, so the same reminder can be tapped again and a later reload of the reminders cannot re-run the scroll (it jumped the tab to the top after every chat turn). The row's mark stays until **Talk about this**.

**A re-read keeps an unsent turn.** When the pass store writes to the thread while the Coach tab is mounted, the tab re-reads it. A turn that failed before producing anything has no row, only a bubble with Retry, and the re-read used to drop it, so a tapped nudge's line sat under his unanswered question as if it answered it. `keepUnsentTail` carries the bubble over, after the new line, where Retry still reaches the question.

**Settings › Coach › Notifications.** **Coach nudges** on/off (on by default — Q1; off cancels every nudge still ahead at once and takes the instructions out of the pass); **Quiet from** / **Quiet until** on the protocol editor's wheel (moving them over a planned nudge HOLDS it back — not listed, not scheduled, not told to the pass — and it returns if they move off it; cancelling it was the first version, and the wheel commits on every settle, so a spin through 19:00 on the way to 22:00 would have destroyed it); **Morning check-in**, off until he turns it on — a daily notification reading only "Morning check-in", whose tap has the Coach speak first after the Health sync settles. The margin note says how to hide the line itself: iOS's Show Previews › When Unlocked for ARC.

**From chat, since 2026-09-25 (no tool added, `docs/coach-domains.md` §10c).** The Coach can do all of the above in conversation, through the domain registry and the same functions the screens call. The four controls are fields of the `settings` domain, written through `saveNudgeSettings`, and the card says old → new in the screen's words (`Quiet hours 21:30–07:00 → 22:00–06:30`, `Morning check-in off → 07:30`, `Coach nudges on → off, which cancels 2 planned notifications`). A planned nudge is cancelled with `edit_record { domain: "nudges", status: "cancelled" }` through the Coach tab's `cancelNudge`, behind a card naming its line and time. `list_reminders` now hands out each planned nudge's id. Moving quiet hours off a held-back nudge says so on the card (`, which lets 1 held-back notification go out`). The Coach can cancel only what the tab lists: a nudge that went out, was opened, was replaced by a newer plan or is held back by quiet hours refuses (a held-back one whose time has passed refuses as held back, not as gone out), and one that fires while the cancel card is open refuses past the gate. The off switch is guarded the same way: its card counts what it cancels at the moment of approval, so a nudge that fires while it is open changes the count and the write refuses, and its write cancels only what is still ahead of the approval, as the screen's switch does at the tap. The context carries the clock source for these checks only; it never derives a day or time that is written. It cannot plan a nudge from chat; that stays the pass's. Cost: +10 schema tokens (9,093 → 9,103 of 9,250), prompt and pass prefix unchanged.

**Also fixed on the way: "Superseded" on a finished reply.** `markSupersededTurns` marked every assistant row followed by another assistant row as replaced by a retry. The coach pass has always appended its note on its own, and a tapped nudge and a plan record now do too, so a finished reply above one of them read "Superseded — replaced by the reply below", which was false. A turn whose outcome is `complete` is never superseded now; only an unfinished one can have been retried.

**Cost.** The chat Coach: `set_reminder`'s `checkin` flag, +19 schema tokens net (9,039 → 9,058 of 9,250), paid partly by deleting two property descriptions that restated the tool description; prompt and pass prefix unchanged. Money: nudges ride passes that already run; the evening pass adds about $0.02 on a day ARC is opened after 18:00 (about $0.65 a month); a check-in tap about $0.02, spent while he is looking.

**What only the phone can settle.** Whether a nudge fires on the lock screen at the planned time, and what iOS does to it under Focus and Sleep; the first-nudge permission prompt landing mid-session; the tap routes on a cold start while the app lock is up (the check-in must wait for Face ID, then answer); the wheel rows in Settings; and how the "Scheduled by the Coach" plate reads under the Reminders plate. All of it needs the next EAS build — no native module was added, but there is no OTA.
...
3. **Evening accountability (shipped 2026-09-25, 0064):** the evening pass runs on the first open after 18:00 while Coach nudges are on, compares what the day planned against what happened, and is the pass that plans tomorrow's notifications. The morning check-in is the opt-in doorbell. Quiet hours are set in Settings › Coach (§3 › Coach notifications).
...
5. **Mid-day corrections (shipped):** `adjust_today` gives the Coach a real lever on the current day, behind one confirmation. As of 2026-09-19 it is the ONLY one — `set_status` records the fact and reshapes nothing, which is the point: what a sick day should contain is a judgment, made on the day, with the user present.
...
**Planned (sequenced):**
1. ~~**Coach notes**~~ — **SHIPPED at 0030**, and the `⚑ MATT` that rode with it (*"where should this be visible/editable? Settings vs Data"*) is **answered: Data › Knowledge base**, by owner decision at C14 (2026-09-14). The store is unchanged — one-sentence durable facts, `remember`/`forget`, injected verbatim every turn, capped at `MEMORY_PROMPT_LIMIT` — and no migration was needed. What moved is the surface: the list is the first run of `app/knowledge.tsx`, beside the knowledge entries it has to be told apart from, and `app/coach-memory.tsx` became the editor for one memory (write, edit, forget, restore, delete). Settings › Coach memory keeps the row and the tally as a **link**, never a second list. The owner can now write a memory himself (`source='user'`), which he could not before. Rationale and the full audit: `docs/knowledge-subapp.md` §2c.
...
- ~~**The tool chips cannot tell two reminder tools apart.**~~ **FIXED 2026-08-25.** `humanizeToolName` stripped a leading `get|list|log|set|complete|dismiss`, and the damage was wider than this entry recorded: **three** tools collided on `"reminder"` (`set_`, `complete_` **and** `dismiss_reminder`) and a second pair on `"recipe"` (`get_` / `log_recipe`). Not cosmetic, for the reason this entry already gave — that string is the receipt fallback for writes recorded before `CoachToolCall.receipt` existed, so *creating a nudge*, *closing one out* and *throwing one away* shared one line of audit trail. **Only the READ verbs are stripped now** (`get`, `list`), which resolves both collisions and replaces "drop the verb" with a rule worth having: **a read chip is a NOUN** (what was consulted — `get_metric_series` → "metric series"), **a write chip is a VERB PHRASE** (what was done — `set_reminder` → "set reminder"). `save_recipe` already read that way; it was the rest of the writes that were wrong. The function moved from `src/lib/ai/coach-service.ts` to the tool registry (`src/lib/ai/tools/index.ts`) — where the vocabulary belongs, and where a headless test can reach it: **`db/coach-tools.test.mjs` §0 asserts the mapping is injective over all 43 registered tools**, so the next collision fails the suite rather than being found on a device. **No prompt cost:** chips are rendered client-side from `ai_messages.tool_calls`; nothing reaches the wire and the §6 token ceilings in `db/coach-eval.test.mjs` are untouched.

**Product decisions for Matt:** every `⚑ MATT` above — ruthlessness rope (§2d), thresholds/targets ownership (§3), ~~memory visibility (§4)~~ *(settled at C14: Data › Knowledge base)*, accountability cadence (§3), read-gating/write-graduation (§5). Plus: default model choice (currently `claude-opus-5`) and whether per-turn token spend should be surfaced in the UI.
