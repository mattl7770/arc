...
- **Parsing** (`parseNudgeReply`, `src/lib/notifications/nudge-plan.ts`). Every line whose first word is `NUDGE` (capitals; bullets and emphasis tolerated) is taken OUT of the reply whether it parses or not — the `isPassSkip` lesson: a sentinel that leaks reaches the owner as the Coach's own words. What remains alone decides spoke vs silent, so `SKIP` plus a nudge line is a silent pass that planned one. A day in words, an impossible date or a 25th hour is malformed and dropped; the day is never inferred.
- **The caps** (`planNudges`) — each refusal drops the line, never moves it: at most **2 per logical day**, counting nudges already sent that day (Q1); **no digit** in the text, enforced in code as well as asked for (Q3: the line shows in full on the lock screen, numbers kept out); at most **140 characters**; at least **5 minutes** ahead and at most **36 hours**; nothing inside **quiet hours** (Q2, default 21:30–07:00); no second nudge in the same slot, and no repeat of words already sent that day. When a pass writes three for one day, the two it wrote first are kept.
...
3. **Numbers are kept off the lock screen by code**, not only by the directive. A digit drops the line.
...
**Taps are routed once.** A cold start can deliver the same response twice and the launching response survives a JS reload; `registerNotificationRouting` remembers each request identifier and clears the launching response once handled. The nudge and check-in paths are idempotent underneath as well.
