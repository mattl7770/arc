...
| `delete_record` | write | `{ domain, id }` — remove ONE row, permanently. **Its own tool over its own smaller enum (12 of the 26 domains)**, because a removal must never be a *value* the model can set in passing beside three other fields on a card that opens with "Edit". **Deletion follows the screens (owner's call, 2026-09-23, superseding the undo-only rule):** every domain in the enum is `hard`, and its `run` is the delete its own screen calls — a meal, a session, a water entry and a protocol included, whoever logged the row. There is no rule about which rows or when; the card is the guardrail. It names the row's day, name and figures (`RemovePolicy.gone`), it is always the long card, and it is re-read past the gate — values and printed line both — so a row that moved refuses. A domain no screen deletes (a capture, a protocol version, a lab report) is absent from the enum and, asked anyway, answers with where the row lives; memories, knowledge, progress photos and reports are held below parity by earlier owner calls (`docs/coach-domains.md` §10a). Every CASCADE out of a removable row is asserted to be its own part (items, sets, versions) against `PRAGMA foreign_key_list` over every table. The consequence lane says the row is deleted for good, with no undo. |
| `edit_record` | write | `{ domain, id, fields }` — patch ONE existing row. **Read-modify-write always**: a field the patch omits is preserved. The card prints before → after, resolved from the row, and throws at card time for anything knowable. `fields` is the registry's **only open schema** (the outer object is still `additionalProperties: false`); the vocabulary is taught by the description, by `query_records`' `fields`, and by an unknown-field error that names the set — never by 26 domains of field names in the cached prefix. Without an `id` it refuses, **naming the bespoke tool** that creates there: prefer-the-specific-tool is a parser rule, not only prompt copy. |
