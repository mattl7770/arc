...
> **The registry today: 40 tools — 19 read + 21 write.** `COACH_TOOLS = [...READ_TOOLS, ...WRITE_TOOLS]` (`src/lib/ai/tools/index.ts`) is the single source of truth; this doc is the spec. §2a and §2b below list the original slice (9 read + 10 write); the ones added since are in **§2c** (statuses, experiments, knowledge, nutrition targets, screenings), **§2f** (recipes + grocery, 3 read + 5 write) and **§2g** (the generic tools over the domain registry), and the deliberately withheld ones are in **§2d**. Counted 2026-09-19 by importing the registry, not by adding up this document — the count says 39 rather than 44 because the fold below *removed* six tools and added one.
>
