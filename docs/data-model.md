...
- **A food is measured in `g` **or** `ml`, and the two never convert** (`0047`, backlog B2). `foods.basis` says which; the food's per-100 macro columns are per 100 **of that basis**, and `meal_items.unit` / `meal_template_items.unit` snapshot it beside the portion at log time. There is no ml↔g factor anywhere in the codebase, because one would need a density per food (milk 1.03, oil 0.92, honey 1.42) that nobody here has measured. It costs nothing: `meal_items.kcal` and the macro columns are absolute amounts **for the portion**, so a day's totals sum across units by construction and no aggregate, index or trigger knows the unit exists. The vocabulary is closed at two — `oz` is a *display* preference (Settings › Units), never a stored unit.
  - The same migration renamed the three portion columns to what they now hold: `meal_items.grams` → `amount`, `meal_template_items.grams` → `amount`, `foods.serving_grams` → `serving_amount`. A column called `grams` holding `250` for a 250 ml drink is the class of lie this schema avoids elsewhere (`meal_photos.file_name` is a *name*, not a path; `wearable_data.metric_type` is free text on purpose).
  - **The per-100 macro columns kept their names** (`kcal_100g`, `protein_g_100g`, …). They are read as "per 100 of the basis", and the basis is one column away on the same row; a portion is different in kind, because it is the number the user types and the screen prints back.
  - **`recipe_ingredients.grams` was deliberately left alone** and gains no unit. A recipe line is parsed from prose ("1 cup milk") and *resolved* to a mass — that resolution to one unit is what makes a recipe's nutrition summable — so its grams really are grams. This also leaves the 0034 invariant `resolved_by IS NULL ⇔ grams IS NULL` untouched: same columns, same writers, same pairing. A millilitre meal item saved into the recipe book (`saveMealAsRecipe`) therefore lands as an **unresolved** line whose raw text states the volume ("250 ml Milk"), to be priced in grams by the same automatic pass every hand-typed volumetric line already goes through.

- **`exercises.measures` is a CHECK'd text of ALL FIFTEEN subsets, not JSON** (0046, 2026-09-14). A movement declares which of `reps · load · time · distance` its sets record, stored as the comma-joined subset in that fixed order — `'reps,load'` for a bench press, `'time'` for a plank, `'time,distance'` for a run, `'load,distance'` for a carry. This is the opposite call from `metric_type` and for the opposite reason: ARC authors the whole vocabulary, so per CLAUDE.md §9 it takes the CHECK. The CHECK is *total* over the four measures — all fifteen non-empty subsets are legal — which is what guarantees it never needs the twelve-step table rebuild that adding a value to `logging_type`'s CHECK would need. The corresponding set columns (`workout_sets.reps` / `weight_kg` / `duration_sec` / `distance_m`) are all nullable, and **"a set carries the fields its exercise implies" is a repository rule, not a CHECK** — the 0034 lesson: a cross-column CHECK passes on an empty fixture and then rejects the ALTER on a populated device. `distance_m` is canonical metres, matching how `weight_kg` is canonical kg, so the km/mi preference is never a migration. Full rationale: `docs/exercise-subapp.md` §10.
- **`exercises.load_basis` stores only the owner's correction, never the derivation** (0062, 2026-09-23). What a movement's weight figure counts — `total`, `per_hand`, `per_side`, `stack`, `bodyweight_plus`, `assisted` — is derived at read time from the catalog row by one pure function (`deriveLoadBasis`, `src/lib/exercise/load-basis.ts`); the column is NULL for ARC's reading and holds a value only where the owner disagreed, and choosing ARC's reading writes NULL again. A stored copy of a derived value would be a second definition that stops improving the day it is written. A correction relabels and never rescales: no `workout_sets` row is touched. Vocabulary-only CHECK, closed at six, because `exercises` parents three foreign keys. Full rationale: `docs/exercise-subapp.md` §15.
