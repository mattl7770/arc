...
- **Backup: the encrypted snapshot exists (2026-08-25, `docs/backups-subapp.md`).** An **XChaCha20-Poly1305-sealed copy of the SQLite file** (ARCB1 container, made with `VACUUM INTO`) is written to `Documents/backups/`, which **deliberately RIDES the iCloud/Finder device backup** — the cloud carries ciphertext only, so §2 holds. Key: 32 bytes in the Keychain at `WHEN_UNLOCKED` (**not** `THIS_DEVICE_ONLY` — it must migrate through an encrypted device backup, the deliberate opposite of the API key), plus a one-time recovery code. Pure-JS crypto (`@noble/ciphers`/`@noble/hashes`), so no EAS gate. **The 2026-08-23 privacy exclusions stand and are permanent: `arc.db` and the photo dirs are EXCLUDED from the device backup** via the `ArcBackup` native seam, since an unencrypted copy would violate §2 — `backup-file-store.ts` must never call that seam. Photos are out of v1 (originals live in iOS Photos; meal photos are transient). Media referenced from iOS Photos (PhotoKit).
...
Deletion is a separate tool over a smaller enum, because a removal must never be a *value* the model can set in passing — and it follows **the same parity rule as everything else** (owner's call, 2026-09-23, reversing the 2026-09-19 undo-only rule): what a screen can delete, the Coach may delete, through the function that screen calls — a meal or a session the user logged by hand included. **The guardrail is the card, and it is mandatory:** it names the row's date, name and figures before the gate, it is never the brief card and never approved on anyone's behalf, and the re-read after the gate refuses a row that moved. *When* to delete is the model's judgment; there is no rule beyond parity. What no screen deletes (a protocol version, a lab report) refuses and names where the row lives. **Since 2026-09-25 (the owner's parity answers, `docs/coach-domains.md` §10b)** parity holds both ways: a catalog food is deletable on Add food too (with an Undo); a Log-tab capture has a × with an Undo, and the Coach removes one through the same `removeLogCapture`, which also takes any copy ARC published to Apple Health; memories and knowledge entries are deletable through `deleteMemory` / `deleteKnowledgeEntry` — a knowledge entry only once archived, because its screen deletes from the Archived list alone. **Progress photos and reports alone stay below parity** (read-only by Q4(a), kept by the owner). The one removal that rides an edit is the Eat tab's combine — `edit_record` on `meals` with `combine_with: [ids]` runs `combineMeals`, which folds the others into one meal and deletes their rows — so its card lists every meal and ends by saying the others are deleted and there is no undo.
...
- `/docs/knowledge-subapp.md` — **BUILT 2026-08-12 (migration 0038, authored as 0035 and renumbered at merge):** the browsable/writable knowledge base over the 0025 RAG substrate — `knowledge_entries`, four Data-pushed screens, article import (URL + paste, its ADR in decisions.md), and the `save_knowledge_entry` Coach tool (registry 43). Headless-verified only
- `/docs/reports-subapp.md` — **BUILT 2026-08-12 (migration 0039, authored as 0036 and renumbered at merge):** the periodic self-review + doctor-visit pack Read it before touching anything under `src/lib/reports/`: the load-bearing rule is that **the renderers compute nothing** — both the HTML file and the native preview consume one typed `ReportData` and every figure is a field, which is what makes them incapable of disagreeing — and **no model prose ever enters the doctor pack**. Headless-verified only
- `/docs/backups-subapp.md` — **BUILT 2026-08-25 (no migration):** the ARCB1 encrypted snapshot — the container format and its AAD rule, the Keychain key's `WHEN_UNLOCKED` accessibility and why it is the opposite of the API key's, the recovery code, the backup/restore/relaunch flows, and why photos are out of v1. Read it before touching anything under `src/lib/backup/`; the load-bearing rule is that this directory **rides** the device backup and must never be excluded from it
