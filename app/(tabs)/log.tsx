 ...
 * an input), "Logged today" is a **ruled plate** (a record is a table), and the
 * symptom row is its own small plate. The quick-adds are a grid, which draws no
 * rules at all. The screen's single accent is the command field's send/mic
 * action — the tiles are deliberately neutral, because none of them is "the one
 * next action".
 */
  ...
  return (
    <Screen scroll>
      ...
      {/* Symptom capture — a distinct "something's off" entry, kept separate from
          ...
          Restored 2026-08-10. The de-plating sweep took this box out on an
          invented rule ("one row is not a record"); the owner rejected it — the
          stray rectangles reported on hardware were a NativeWind divider
          artefact, not this enclosure. */}
      <View className="mt-5">
        <Block device="plate">
          <Pressable
