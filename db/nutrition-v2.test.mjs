...
console.log('36. C4/C5: the estimator prompts have a ceiling now');
{
  ...
  // more — the ceiling did NOT move (§59 pins it):
  //
  //   +41  the rest of the shortlist, only where a portion gives 10%+ of a
  //        day's value
  //   +10  "micros" on a composite's parts (§58)
...
console.log('38. C5: the four effects, as on-device arithmetic');
{
  ...
  // since 2026-09-23, replays every LATER answer on top of — §57).
...
console.log('53. feedback: the bar counts every figure the reply carries, not energy alone');
{
  ...
  // (The bullet was rewritten around its keys on 2026-09-23 — §59 — and still
...
console.log('54. feedback: a shots answer reaches the caffeine figure, not only the kcal');
{
  ...
  // Changing the answer re-applies to the same base (`answerQuestion`, §57), so
...
// ===========================================================================
// 56–58. Micros, round 2 (owner notes 2026-09-23) — the three defects the
...
console.log('56. defect: a matched food with no micros keeps the model’s figures through review');
{
  const { db } = freshDb();
...
console.log('57. defect: two answered questions compose — changing the first keeps the second');
{
...
console.log('58. defect: a composite’s parts carry sodium and caffeine');
{
...
// ===========================================================================
// 59. "Other micronutrients should start getting something" (owner,
...
// source" line), and grounding merges a catalog food's micros key by key (§56).
// ===========================================================================
console.log('59. the estimator asks for the shortlist where a portion is a notable source');
{
  const E = MEAL_ESTIMATION_SYSTEM_PROMPT;
  const R = MEAL_REVISION_SYSTEM_PROMPT;
  const bullet = (p) =>
...
// ===========================================================================
// 60. The one notable micro on an item row (owner: "displaying caffeine on a
// latte"). One pure function, keyMicro (src/lib/nutrition/key-micro.ts).
// ===========================================================================
console.log('60. keyMicro: caffeine whenever present, sodium above a fifth, fiber above 5 g');
{
...
// ===========================================================================
// 61. The day's three under the Eat tab's macro bars (owner: "Sodium and
// caffeine more accessible", "Fiber should be more visible too").
// ===========================================================================
console.log('61. dayKeyMicros: sodium and caffeine against ceilings, fiber against his target');
{
