 ...
 * exception to "a protocol edited today reshapes only tomorrow". This holds for
 * EVERY field, not just the title: a kept row (matched by title + protocol) is
 * re-synced in place to the plan's current dose/why/scheduled_time, so a pure
 * dose edit propagates as surely as a renamed item does. Pending rows only;
 * nothing logged is touched. Recording the generating version id on each row
 * would remove the exception, and is the fix if it ever bites.
 */
